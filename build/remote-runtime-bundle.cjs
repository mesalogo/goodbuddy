...
const { targetName, assertTargetBinary } = require("./agent-build-target.cjs");
const {
  preflightRegisteredProductionKey,
} = require("./signing-key-preflight.cjs");
const { prepareBundledOpenCodeConfig } = require("./opencode-config.cjs");

const root = join(__dirname, "..");
const supportedArchitectures = Object.freeze(["x64", "arm64"]);
const manifestFileName = "manifest.json";
const signatureFileName = "manifest.sig";
const signingPrivateKeyEnvironment = "GOODBUDDY_SIGNING_PRIVATE_KEY";
const signingKeyIdEnvironment = "GOODBUDDY_SIGNING_KEY_ID";
...
function readRemoteRuntimeLock(projectRoot = root) {
  ...
  for (const architecture of supportedArchitectures) {
    ...
    if (
      ...
      throw new Error(`Remote Runtime lock target is invalid: ${architecture}`);
    }
  }
  return lock;
}
...
function trustedKeyForSigning(
  ...
  if (
    registry.revocations.some((revocation) => revocation.keyId === record.keyId)
  ) {
...
function preflightProductionSigningKey(options = {}) {
  ...
  return preflightRegisteredProductionKey({
    ...
    keyId,
    registry,
    missingKeyIdMessage: `${signingKeyIdEnvironment} is not configured; provision the production GoodBuddy signing key ID before building a release`,
  });
}
...
function validateManifestShape(manifest) {
  if (
    ...
    !/^sha256:[a-f0-9]{64}$/u.test(manifest.acpCapabilitiesDigest) ||
...
function trustedKeyForManifest(manifest, registry, verificationEnvironment) {
  ...
  if (
    registry.revocations.some((revocation) => revocation.keyId === key.keyId)
  ) {
    throw new Error("Runtime signing key is outside its trusted interval");
  }
  return key;
...
function assertManifestMatchesLock(
  manifest,
  lock,
  architecture,
  platform = "linux",
...
function verifyBundleDirectory(bundleDirectoryInput, options = {}) {
  ...
  const manifestBytes = readFileSync(join(bundleDirectory, manifestFileName));
  ...
  if (!canonicalManifestBytes(manifest).equals(manifestBytes)) {
    throw new Error("Runtime manifest is not in canonical deterministic form");
  }
  assertManifestMatchesLock(manifest, lock, architecture, platform);
  const environment = options.verificationEnvironment ?? "production";
  const key = trustedKeyForManifest(manifest, registry, environment);
  const publicKey = createPublicKey({
    key: Buffer.from(key.publicKeySpkiBase64, "base64"),
    format: "der",
    type: "spki",
  ...
  if (
    ...
    !verify(null, signaturePayload(manifestBytes), publicKey, signatureBytes)
  ) {
  ...
  if (
    ...
    basename(bundleDirectory) !== manifest.bundleDigest.slice("sha256:".length)
  ) {
    throw new Error(
  ...
  const expected = new Set([...declared, manifestFileName, signatureFileName]);
  if (
  ...
  if (
    ...
    manifest.entrypoint.sha256 !==
      manifest.files.find((file) => file.path === manifest.entrypoint.path)
        ?.sha256
  ) {
...
function buildRuntimeBundle(options) {
  ...
  const verificationEnvironment = options.testSigningIdentity
    ? "test"
    : "production";
  trustedKeyForSigning(signingIdentity, registry, verificationEnvironment);
  const outputRoot = options.outputRoot ?? defaultOutputRoot(projectRoot);
  ...
  try {
    ...
    if (
      ...
      if (sourceMetadata.name !== sourcePackage.package) {
...
function parseArguments(argv) {
  ...
  for (let index = 1; index < argv.length; index += 2) {
    ...
    options[
      flag
        .slice(2)
        .replace(/-([a-z])/gu, (_match, value) => value.toUpperCase())
    ] = value;
  }
