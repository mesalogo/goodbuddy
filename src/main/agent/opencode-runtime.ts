...
const MAX_NATIVE_MCP_SERVERS = runtimeNativeInventoryLimits.mcpServers;
...
const temporaryMcpDenyRules: PermissionRuleset = TEMPORARY_MCP_PREFIXES.map(
  (prefix) => ({
    permission: `${prefix}*`,
    pattern: "*",
    action: "deny",
...
function resolveOpenCodeProvider(
  ...
  if (profile.protocol === "openai-images-generations") {
    throw new Error("OpenCode 独立模型连接不支持图像生成协议");
  }
...
function createOpenCodeProviderConfig(
  ...
  const options: {
    ...
    ...(profile.requestHeaders && Object.keys(profile.requestHeaders).length > 0
...
function areBoundedPatterns(value: unknown): value is string[] {
  return (
    ...
    byteLengthWithin(value.join("\0"), MAX_PERMISSION_PATTERNS_BYTES)
  );
}
...
function parsePermissionRequest(
  ...
  const { id, permission, patterns, metadata, always, tool } = properties;
  if (
    typeof id !== "string" ||
    id.length === 0 ||
  ...
  if (!byteLengthWithin(serializedMetadata, MAX_PERMISSION_METADATA_BYTES)) {
...
function boundedResourceUri(value: unknown): string | undefined {
  ...
  try {
    ...
    return bounded.includes("?") || bounded.includes("#") ? undefined : bounded;
  }
}
...
async function normalizeOpenCodeSkillManifest(
  ...
  await writeFile(
    manifestPath,
    ["---", stringifyYaml(normalizedMetadata).trimEnd(), "---", match[2]].join(
      "\n",
    ),
    "utf8",
  );
...
export class OpenCodeRuntime implements AgentRuntime {
  ...
  private getNativeSkillIds(): string[] {
    ...
    const ids = (this.options.skillPackages ?? []).map((skill) => skill.id);
    if (
      new Set(ids).size !== ids.length ||
      ids.some((id) => id.length > 64 || !OPENCODE_SKILL_NAME_PATTERN.test(id))
    ) {
  ...
  private async mutateMcp<T>(
    signal: AbortSignal,
    operation: () => Promise<T>,
  ): Promise<T> {
    signal.throwIfAborted();
    const previous = this.mcpMutationTail;
    let release!: () => void;
    const current = new Promise<void>((resolve) => {
      release = resolve;
    });
    this.mcpMutationTail = previous.then(
      () => current,
      () => current,
    );
    try {
  ...
  private async createSkillRegistration(): Promise<OpenCodeSkillRegistration> {
    ...
    try {
      ...
      if (sharedRoot) {
        const configDirectory = join(resolve(sharedRoot), "config", "opencode");
        await this.prepareSharedConfig(configDirectory);
        const skillsRoot = await this.prepareSharedSkills(resolve(sharedRoot));
  ...
  private async prepareSharedConfig(configDirectory: string): Promise<void> {
  ...
  private async inspectEmbeddedConfiguration(validatePath = false): Promise<{
    path?: string;
    detail: string;
  }> {
    if (
    ...
    return this.dependencies.validateBinary("opencode", detection.path);
  }
  ...
  private async launchEmbedded(signal?: AbortSignal): Promise<OpenCodeServer> {
    ...
    try {
      ...
      return await new Promise<OpenCodeServer>((resolveServer, reject) => {
        const child = this.dependencies.spawn(binaryPath, serverArgs, {
  ...
  private async controlRequest<T>(
    ...
    const timeout = setTimeout(
      () => {
        timeoutController.abort(new Error(`OpenCode ${label}超时`));
      },
    ...
    try {
      ...
      return await awaitWithAbort(request, controlSignal);
    } catch (error) {
  ...
  private async initializeClient(
    ...
    if (this.client && this.server) {
      ...
      let healthy = false;
      try {
        healthy = await awaitWithAbort(
          this.dependencies.checkServerHealth(
            this.server.url,
        ...
        // A dead or unresponsive cached server must not poison later requests.
      }
      signal?.throwIfAborted();
  ...
  async getStatus(): Promise<AgentRuntimeStatus> {
    if (this.usesEmbeddedPermissionMediation() && !this.client) {
      try {
        const detection = await this.inspectEmbeddedConfiguration(true);
        return {
          id: "opencode",
          label: "OpenCode",
  ...
  > {
    ...
    return response.data
      .flatMap((agent) => {
        ...
        const description = boundedNativeText(agent.description, 2_000);
        return [
          {
            id,
            name: id.slice(0, 200),
  ...
  private async resolveSelectedAgent(
    ...
    const control =
      request.runtimeControl?.provider === "opencode"
        ...
        : undefined;
    const selected = control?.agent ?? this.options.customization?.defaultAgent;
    if (!selected) {
      return undefined;
    }
  ...
  async getNativeSnapshot(): Promise<RuntimeNativeSnapshot> {
    ...
    const empty: RuntimeNativeSnapshot = {
      provider: "opencode",
      available: false,
      inventoryStatus: controlled ? "unavailable" : "connection-only",
      detail: controlled
        ? "OpenCode 原生能力不可用"
    ...
    const [
      ...
      this.controlRequest("读取资源清单", (signal) =>
        client.experimental.resource.list({ directory }, { signal }),
      ),
    ]);
    if (
    ...
    const commands = nativeCommandData.flatMap((command) => {
      ...
      const description = boundedNativeText(command.description, 2_000);
    ...
    const skills = skillData
      .flatMap((skill) => {
        ...
        const description = boundedNativeText(skill.description, 2_000);
        return [
          {
            id,
            name: id.slice(0, 200),
    ...
    const resources = Object.entries(resourceData)
      .flatMap(([rawId, resource]) => {
        ...
        const description = boundedNativeText(resource.description, 2_000);
    ...
    return {
      provider: "opencode",
      available: true,
      inventoryStatus: unavailable.length === 0 ? "available" : "partial",
      detail:
  ...
  async *run(
    ...
    const deadline =
      configuredTimeout === undefined ? undefined : new AbortController();
    const deadlineTimer =
  ...
  private async *runWithClient(
    ...
    const runtimeControl =
      request.runtimeControl?.provider === "opencode"
        ? request.runtimeControl
        : undefined;
    if (runtimeControl?.command && !this.supportsNativeCustomization()) {
      throw new Error("外部 OpenCode Server 不支持由 GoodBuddy 执行原生命令");
    }
    ...
    if (runtimeControl?.command) {
      const commandResponse = await this.controlRequest(
        "验证原生命令",
        (controlSignal) =>
          client.command.list({ directory }, { signal: controlSignal }),
        signal,
      );
      ...
      const command = commandResponse.data.find((candidate) => {
    ...
    try {
      ...
      if (
        ...
        const customMcpEndpoint = this.options.knowledgeGateway.getEndpoint()!;
      ...
      const retainOutputDelta = (
        ...
        const remaining = MAX_EXECUTION_OUTPUT_BYTES - aggregateOutputBytes;
      ...
      try {
          ...
          ? client.session.command(
              {
                ...
                ...(this.options.modelProfile
                  ? {
                      model: `${
                        resolveOpenCodeProvider(this.options.modelProfile).id
        ...
        for await (const event of subscription.stream) {
          ...
          if (
            ...
            if (reasoning || event.properties.field === "text") {
              if (
                ...
                [...toolStates.values()].some((tool) => tool.state === "failed")
              ) {
          ...
          if (
            ...
            if (
              ...
              reportedQuestionIds.set(questionRequest.id, publicQuestionId);
          ...
          if (
            ...
            throw new Error(opencodeErrorMessage(error, "OpenCode 执行失败"));
          }
          ...
          if (
            ...
            if (incompleteTool) {
              ...
              throw new Error(`OpenCode 工具未完成（${callId.slice(0, 128)}）`);
            }
            const failedTools = [...toolStates.entries()].filter(
              ([, tool]) => tool.state === "failed",
            );
            if (failedTools.length > 0 && !hasResponseTextAfterFailure) {
        ...
        if (promptResult.error) {
          throw new Error(
            opencodeErrorMessage(promptResult.error, "OpenCode 提交请求失败"),
          );
        }
        signal.throwIfAborted();
  ...
  async respondToQuestion(
    ...
    const response = answers
      ? answers.length === pending.questionCount
        ? await this.controlRequest("提交提问回答", (signal) =>
        ...
        : undefined
      : await this.controlRequest("取消提问", (signal) =>
  ...
  async compactConversation(
    ...
    try {
      ...
      const configuredModel = this.options.modelProfile
        ? {
            providerID: resolveOpenCodeProvider(this.options.modelProfile).id,
            modelID: this.options.modelProfile.modelName,
          }
      ...
      try {
        ...
        if (compact.error || compact.data !== true) {
          throw new Error(
            opencodeErrorMessage(compact.error, "OpenCode 原生 Compact 失败"),
          );
        }
