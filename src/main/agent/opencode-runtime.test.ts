...
function runClient(events: Record<string, unknown>[]) {
  ...
  const client = {
    ...
    mcp: {
      add: vi.fn().mockImplementation(async (input: { name: string }) => ({
        data: {
...
function embeddedRuntime(
  ...
  const { deps } = dependencies(child, {
    ...dependencyOverrides,
    createClient: vi.fn(() => client) as unknown as typeof createOpencodeClient,
  });
...
describe("OpenCodeRuntime embedded launcher", () => {
  it("checks embedded availability without starting OpenCode", async () => {
    ...
    expect(detectBinary).toHaveBeenCalledWith("opencode", "", undefined);
    expect(validateBinary).toHaveBeenCalledWith("opencode", "opencode");
  ...
  it("uses the detected binary and passes an absolute config path only through env", async () => {
    ...
    await expect(runtime.testConnection()).resolves.toMatchObject({
      available: true,
      detail: "由 GoodBuddy 以当前用户权限管理本机 OpenCode 进程",
    });
    expect(detectBinary).toHaveBeenCalledWith(
      "opencode",
    ...
    expect(spawnMock).toHaveBeenNthCalledWith(
      ...
      ["serve", "--hostname=127.0.0.1", expect.stringMatching(/^--port=\d+$/u)],
    ...
    const spawnOptions = (
      ...
      >
    )[0]?.[2] as { env?: NodeJS.ProcessEnv } | undefined;
    expect(spawnOptions?.env?.OPENCODE_SERVER_USERNAME).toBe("goodbuddy");
    expect(spawnOptions?.env?.OPENCODE_SERVER_PASSWORD).toBeTruthy();
    expect(spawnOptions?.env?.PATH).toBe("C:\\GoodBuddy\\tools;C:\\Windows");
    ...
    ).toBe(`goodbuddy:${spawnOptions?.env?.OPENCODE_SERVER_PASSWORD}`);
  ...
  it("registers only assigned Skill packages in an isolated config directory", async () => {
    ...
    const registeredSkill = join(configDirectory!, "skills", "longdoc-docx");
    try {
      await expect(
        readFile(join(registeredSkill, "templates", "document.txt"), "utf8"),
  ...
  it("reuses shared OpenCode config and content-addressed Skills across projects", async () => {
    ...
    try {
      ...
      expect(secondEnvironment?.OPENCODE_CONFIG_DIR).toBe(firstConfigDirectory);
  ...
  it("isolates an explicit profile from unrelated inherited credentials", async () => {
    ...
    try {
      ...
      const environment = (
        spawnMock.mock.calls[0]?.[2] as { env?: NodeJS.ProcessEnv } | undefined
      )?.env;
  ...
  it.each([
    ...
    async ({ protocol, expectedPath, unexpectedPath }) => {
      const root = await mkdtemp(join(tmpdir(), "goodbuddy-opencode-routing-"));
  ...
  it.each([
    ...
    async ({ protocol, authentication, providerId, providerPackage }) => {
      const child = fakeChild();
      const { deps, spawnMock } = dependencies(child);
      const runtime = new OpenCodeRuntime(
        options({
          modelProfile: {
            ...
            ...(authentication === "api-key" ? { apiKey: "private-key" } : {}),
          },
        }),
        deps,
      );
      ...
      if (authentication === "api-key") {
        expect(config.provider?.[providerId]?.options?.apiKey).toBe(
          "private-key",
        );
      } else {
        expect(config.provider?.[providerId]?.options).not.toHaveProperty(
          "apiKey",
        );
      }
  ...
  it("isolates embedded server configuration from inherited env", async () => {
    ...
    process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://telemetry.invalid";
    try {
      const runtime = new OpenCodeRuntime(options(), deps);
      await expect(runtime.testConnection()).resolves.toMatchObject({
        available: true,
      ...
      expect(spawnOptions?.env?.OPENCODE_CONFIG_CONTENT).not.toBe(
      ...
      expect(spawnOptions?.env?.OPENCODE_SERVER_USERNAME).toBe("goodbuddy");
      expect(spawnOptions?.env?.OPENCODE_SERVER_PASSWORD).not.toBe(
        "must-not-be-inherited",
      );
      expect(spawnOptions?.env).toMatchObject({
        DO_NOT_TRACK: "1",
  ...
  it("polls the known loopback URL without waiting for stdout", async () => {
    ...
    checkServerHealth.mockResolvedValueOnce(false).mockResolvedValueOnce(true);
    const runtime = new OpenCodeRuntime(options(), deps);
    ...
    const [url, authorization, signal] = checkServerHealth.mock.calls[0] ?? [];
  ...
  it("keeps a healthy cached server and client after idle", async () => {
    const child = fakeChild();
    const { deps, checkServerHealth, createClient, spawnMock } = dependencies(child);
    const runtime = new OpenCodeRuntime(options(), deps);
    try {
  ...
  it.each(["unhealthy", "rejected", "hung"])(
    ...
    async (failure) => {
      ...
      try {
        await runtime.testConnection();
        if (failure === "hung") {
          checkServerHealth.mockImplementationOnce(() => new Promise(() => {}));
        } else if (failure === "rejected") {
          checkServerHealth.mockRejectedValueOnce(new Error("fetch failed"));
        } else {
          checkServerHealth.mockResolvedValueOnce(false);
        }
        ...
        expect(checkServerHealth).toHaveBeenCalledTimes(3);
      } finally {
        await runtime.dispose();
      }
    },
  ...
  it("creates a new session after replacing a stale server without replaying prompts", async () => {
  ...
  it("isolates cancellation while concurrent requests wait for a cached health check", async () => {
    const setup = runClient([
      { id: "idle", type: "session.idle", properties: { sessionID: "session-1" } },
    ]);
    const child = fakeChild();
    ...
    try {
      await runtime.testConnection();
      const controller = new AbortController();
      checkServerHealth.mockReturnValueOnce(health);
      const stream = runtime.run(
        { requestId: "cancelled", conversationId: "conversation-cancelled", prompt: "test" },
        controller.signal,
      );
      const cancelled = stream.next().catch((error: unknown) => error);
      runs.push(cancelled);
      await vi.waitFor(() => expect(checkServerHealth).toHaveBeenCalledTimes(2));
      const waiting = collectRun(runtime);
      runs.push(waiting);
      // Let the second request acquire its conversation lock and join the probe.
      await new Promise<void>((resolve) => setImmediate(resolve));
      controller.abort(new Error("cancelled"));
      await expect(cancelled).resolves.toMatchObject({ message: "cancelled" });
      expect(checkServerHealth.mock.calls[1]![2].aborted).toBe(false);
      // A third caller must still join the pending probe after A exits.
      const third = runtime.testConnection();
      runs.push(third);
      expect(checkServerHealth).toHaveBeenCalledTimes(2);
      resolveHealth(true);
      await expect(waiting).resolves.toContainEqual(expect.objectContaining({ type: "done" }));
      await expect(third).resolves.toMatchObject({ available: true });
      expect(child.kill).not.toHaveBeenCalled();
      expect(spawnMock).toHaveBeenCalledOnce();
      expect(checkServerHealth).toHaveBeenCalledTimes(2);
    } finally {
  ...
  it("bounds session setup without imposing a full-run deadline", async () => {
    ...
    await expect(collectRun(runtime)).rejects.toThrow("OpenCode 创建会话超时");
    await runtime.dispose();
  });
  ...
  it("serializes external runs that share one conversation session", async () => {
    ...
    const first = collect(runtime.run(request, new AbortController().signal));
  ...
  it("rejects images when the explicit model connection disables image input", async () => {
    ...
    await expect(stream.next()).rejects.toThrow("当前模型连接未启用图像输入");
    expect(createClient).not.toHaveBeenCalled();
  });
});
...
describe("OpenCodeRuntime embedded permission mediation", () => {
  it("shares assigned custom MCP only with embedded Execute through a scoped loopback token", async () => {
    ...
    expect(
      JSON.stringify(
        (setup.client.mcp.add as unknown as ReturnType<typeof vi.fn>).mock
          .calls,
      ),
  ...
  it("applies the total deadline while agent discovery is stalled", async () => {
    const setup = runClient([]);
    const agents = vi.fn(() => new Promise<never>(() => undefined));
    Object.assign(setup.client, {
      app: { agents },
    });
    const child = fakeChild();
  ...
  it("truncates oversized text display without stopping the run", async () => {
    ...
    expect(events).toContainEqual({
      requestId: "3f496642-f47d-4e0a-8944-a32c77b0d6ef",
      type: "status",
      message: "OpenCode 输出较长，已停止继续展示文本；任务仍在继续",
    });
    expect(events.at(-1)).toMatchObject({ type: "done" });
    expect(setup.session.abort).not.toHaveBeenCalled();
    await runtime.dispose();
  ...
  it("adds only request-scoped built-in read tools for Ask and disconnects them", async () => {
    ...
    toolIds
      ...
      .mockResolvedValueOnce({
        data: ["read", "write", "bash", "goodbuddy_knowledge_search"],
        error: undefined,
      })
      .mockResolvedValue({
        data: ["read", "write", "bash", "goodbuddy_knowledge_search"],
  ...
  it.each(["knowledge", "custom"])("releases the conversation when a queued %s MCP add is cancelled", async (kind) => {
    const setup = runClient([
      { id: "idle", type: "session.idle", properties: { sessionID: "session-1" } },
    ]);
    ...
    add.mockImplementationOnce(async (input: { name: string }) => {
      await registration;
      return { data: { [input.name]: { status: "connected" } } };
    });
    const revoke = vi.fn();
    const runtime = embeddedRuntime(setup.client, {
      knowledgeGateway: {
        getEndpoint: () => "http://127.0.0.1:4567/mcp",
        getAvailableToolNames: () => ["knowledge_search"],
        grantCustomMcp: () => "custom-token",
        prepareCustomMcpTools: async () => [{ name: "custom_tool" }],
        revoke,
      } as unknown as KnowledgeMcpGateway,
      mcpServers: [{
        id: "custom",
    ...
    const mutations = vi.spyOn(runtime as unknown as { mutateMcp: (...args: unknown[]) => Promise<unknown> }, "mutateMcp");
    const controller = new AbortController();
    const collect = async (requestId: string, conversationId: string, signal: AbortSignal, mcp: boolean) => {
      const events: RuntimeEvent[] = [];
      for await (const event of runtime.run({
    ...
    const runs: Promise<unknown>[] = [];
    try {
      runs.push(collect("first", "conversation-first", new AbortController().signal, true));
      await vi.waitFor(() => expect(add).toHaveBeenCalledOnce());
      const cancelled = collect("cancelled", "conversation-second", controller.signal, true).catch((error: unknown) => error);
      runs.push(cancelled);
      await vi.waitFor(() => expect(mutations).toHaveBeenCalledTimes(2));
      controller.abort(new Error("cancelled queued add"));
      let cancellationFinished = false;
      void cancelled.then(() => { cancellationFinished = true; });
      await vi.waitFor(() => expect(cancellationFinished).toBe(true));
      await expect(cancelled).resolves.toMatchObject({ message: "cancelled queued add" });
  ...
  it("keeps cleanup queued behind blocked registration while sessions run concurrently", async () => {
    const setup = runClient([]);
    const releases: (() => void)[] = [];
    ...
    const create = setup.session.create as unknown as ReturnType<typeof vi.fn>;
    let sessionCount = 0;
      ...
      return {
        stream: (async function* () {
          ...
          yield {
            id: `idle-${index}`,
            type: "session.idle",
            properties: { sessionID: `session-${index + 1}` },
          };
        })(),
      };
    });
    ...
    add.mockImplementation(async (input: { name: string }) => {
      if (add.mock.calls.length === 3) await registration;
      return { data: { [input.name]: { status: "connected" } } };
    });
    ...
    const events: RuntimeEvent[] = [];
    const collect = async (index: number): Promise<void> => {
      for await (const event of runtime.run(
        {
    ...
    try {
      runs.push(collect(1));
      await vi.waitFor(() =>
        expect(setup.session.promptAsync).toHaveBeenCalledTimes(1),
      );
      runs.push(collect(2));
      await vi.waitFor(() =>
      ...
      expect(setup.session.promptAsync).toHaveBeenNthCalledWith(
        2,
      ...
      // Exceed the disconnect budget while another registration owns the queue.
      ...
      expect(disconnect.mock.calls[0]![1].signal.aborted).toBe(false);
      await vi.waitFor(() =>
        expect(setup.session.promptAsync).toHaveBeenCalledTimes(3),
      );
      releases[1]!();
      releases[2]!();
      await Promise.all(runs);
      expect(disconnect.mock.calls.map(([input]) => input.name)).toEqual(
        add.mock.calls.map(([input]) => input.name),
      );
  ...
  it("does not send a knowledge capability to external OpenCode", async () => {
    ...
    expect(
      JSON.stringify(
        (setup.session.promptAsync as unknown as ReturnType<typeof vi.fn>).mock
          .calls,
      ),
  ...
  it("configures Execute tools as allowed before prompting", async () => {
    const { client, callOrder, permissionReply, session } = runClient([
    ...
    const { client, session } = runClient([
      {
        ...
        properties: {
          ...
          part: {
            ...
            state: {
              status: "error",
              error: "write failed Authorization: Bearer secret-token",
            },
          },
        },
      },
  ...
  it("keeps a completed response when an earlier tool attempt failed", async () => {
    ...
    expect(
      events.filter(
        (event) => event.type === "tool" && event.callId === "call-1",
      ),
    ).toEqual([
      expect.objectContaining({
        state: "failed",
  ...
  it("surfaces a rejected async prompt instead of reporting success", async () => {
    ...
    vi.mocked(session.promptAsync).mockResolvedValueOnce({
      ...
      error: {
        data: {
          message: "prompt rejected Authorization: Bearer secret-token",
        },
      },
    } as never);
    const runtime = embeddedRuntime(client);
  ...
  it("fails closed when the OpenCode permission reply fails", async () => {
    const { client, permissionReply, session } = runClient([permissionEvent()]);
...
describe("OpenCodeRuntime native customization", () => {
  it("maps bounded native inventory and filters GoodBuddy-owned capabilities", async () => {
    ...
    const assignedSkillDirectory = join(sourceRoot, "assigned-skill");
