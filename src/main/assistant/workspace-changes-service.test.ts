...
describe('getWorkspaceChanges', () => {
  it('reads unborn repositories, nested untracked files, index and worktree diffs separately', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'goodbuddy-changes-'))
    temporaryDirectories.push(directory)
    await execute('git', ['init'], { cwd: directory })
    expect(await getWorkspaceChanges(directory)).toMatchObject({ available: true, files: [], patch: '' })
    await mkdir(join(directory, 'new directory'))
    const path = 'new directory/[file].txt'
    await writeFile(join(directory, path), 'index\n')
    expect((await getWorkspaceChanges(directory)).files).toContainEqual({ path, status: '??' })
    expect((await getWorkspaceChanges(directory, path)).patch).toContain('+index')
    await execute('git', ['add', '--', path], { cwd: directory })
    await writeFile(join(directory, path), 'worktree\n')
    const diff = await getWorkspaceChanges(directory, path)
    expect(diff).toMatchObject({ available: true })
    expect(diff.stagedPatch).toContain('+index')
    expect(diff.patch).toContain('-index')
    expect(diff.patch).toContain('+worktree')
  })

  it('reads deleted and renamed paths literally without including unrelated changes', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'goodbuddy-changes-'))
    temporaryDirectories.push(directory)
    const git = (...args: string[]) => execute('git', args, { cwd: directory })
    await git('init')
    await writeFile(join(directory, 'old name.txt'), 'original\n')
    ...
    await git('mv', 'old name.txt', '[new].txt')
    await writeFile(join(directory, '[new].txt'), 'edited\n')
    await writeFile(join(directory, 'other.txt'), 'unrelated\n')
    const renamed = await getWorkspaceChanges(directory, '[new].txt')
    expect(renamed.stagedPatch).toContain('rename from old name.txt')
    expect(renamed.stagedPatch).toContain('rename to [new].txt')
    expect(renamed.patch).toContain('+edited')
    expect(renamed.patch).not.toContain('unrelated')
    await rm(join(directory, '[new].txt'))
    expect((await getWorkspaceChanges(directory, '[new].txt')).patch).toContain('-original')
    await git('add', '-u')
    expect((await getWorkspaceChanges(directory, 'old name.txt')).stagedPatch).toContain('-original')
  })
