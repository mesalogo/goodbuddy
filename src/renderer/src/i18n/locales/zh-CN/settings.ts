export const settings = {
  ...
  runtime: {
    ...
    advanced: '高级设置',
    sourceLegend: '模型配置来源',
    followRecommended: '跟随全局默认模型',
    goodBuddyModelSource: '使用 GoodBuddy 模型连接',
    goodBuddyConnection: 'GoodBuddy 模型连接',
    pinConnectionDescription: '选择默认项会跟随全局默认模型；选择具体连接会固定使用该连接。',
    ...
    deepseekHarness: {
      ...
      environmentSource:
        '此选项实际使用管理员预置：{{model}}。',
      platformFallback:
        '未配置完整的管理员预置；已保存配置中的实际回退：{{name}} · {{model}}。',
      platformNoFallback:
        '没有可用的管理员预置或兼容的 GoodBuddy 连接。',
      platformSourceUnavailable:
        '模型来源信息不可用，请重新加载设置。',
      platformDraftFallback:
        '草稿兼容候选（未保存）：{{name}} · {{model}}。保存后按管理员环境重新解析来源。',
      platformDraftNoFallback:
        '未保存草稿中没有兼容回退连接。',
      connection: 'OpenAI 兼容模型连接',
      connectionPlaceholder: '自动选择兼容模型连接',
      connectionDescription:
        '“管理员环境优先”先使用部署环境中完整的模型地址、模型名称和 API Key，否则使用 GoodBuddy 默认兼容连接，再回退首个兼容连接。“跟随全局默认模型”跳过管理员预置；指定连接则保持固定。连接必须使用 OpenAI Chat Completions 和 API Key。此选项不加载 Harness 用户 profile 或配置文件。',
      advancedDescription:
