...
describe('ProjectActivity', () => {
  ...
  it('positions beside the anchor, falls left and drills down in a narrow viewport', () => {
    ...
    expect(screen.getByRole('menu', { name: 'Remote' }).style.left).toBe('336px')
  ...
  it('clamps a tall submenu independently without moving or widening the project menu', () => {
    vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(1000)
    vi.spyOn(window, 'innerHeight', 'get').mockReturnValue(600)
    render(view())
    const trigger = screen.getByRole('button', { name: /All project activity/ })
    vi.spyOn(trigger, 'getBoundingClientRect').mockReturnValue(new DOMRect(24, 300, 240, 32))
    open()
    const menu = screen.getByRole('menu', { name: 'All project activity' })
    vi.spyOn(menu, 'offsetHeight', 'get').mockReturnValue(200)
    const remote = screen.getByRole('menuitem', { name: /Remote/ })
    vi.spyOn(remote, 'getBoundingClientRect').mockReturnValue(new DOMRect(32, 410, 248, 52))
    fireEvent.click(remote)
    const sessions = screen.getByRole('menu', { name: 'Remote' })
    vi.spyOn(sessions, 'offsetHeight', 'get').mockReturnValue(100)
    fireEvent(window, new Event('resize'))
    expect(sessions.style.top).toBe('410px')
    vi.spyOn(sessions, 'offsetHeight', 'get').mockReturnValue(480)
    fireEvent(window, new Event('resize'))
    expect(sessions.style.top).toBe('104px')
    expect(sessions.style.left).toBe('288px')
    expect(menu.style.top).toBe('336px')
    expect(menu.style.left).toBe('24px')
    expect(menu.style.width).toBe('264px')
  })

  it('drills down when neither side of the project menu can fit a submenu', () => {
    vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(640)
    render(view())
    const trigger = screen.getByRole('button', { name: /All project activity/ })
    vi.spyOn(trigger, 'getBoundingClientRect').mockReturnValue(new DOMRect(200, 100, 200, 32))
    open()
    fireEvent.click(screen.getByRole('menuitem', { name: /Remote/ }))
    expect(screen.getByRole('menu', { name: 'All project activity' })).toHaveAttribute('data-compact', 'true')
    const sessions = screen.getByRole('menu', { name: 'Remote' })
    expect(sessions.style.left).toBe('')
    expect(sessions.style.top).toBe('')
  })
  ...
  it('uses shared native-browser occlusion only where menus intersect the viewport', () => {
    ...
    fireEvent.click(screen.getByRole('menuitem', { name: /Remote/ }))
    vi.spyOn(screen.getByRole('menu', { name: 'Remote' }), 'getBoundingClientRect')
      .mockReturnValue(new DOMRect(274, 60, 264, 480))
    expect(isBrowserViewportOccluded(host, new DOMRect(400, 300, 500, 500))).toBe(true)
  })
})
