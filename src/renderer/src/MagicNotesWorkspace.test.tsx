...
describe('MagicNotesWorkspace detail behavior', () => {
  ...
  it('retains AI visibility and width across navigation and remounts', async () => {
    ...
    expect(toggle).toHaveTextContent('隐藏 AI 评论')
    ...
    expect(toggle).toHaveTextContent('显示 AI 评论')
  ...
  it('keeps entry editing contained and supports deletion confirmation', async () => {
    const { container } = render(<MagicNotesWorkspace onNotify={onNotify} />)
    await openNote()
    expect(container.querySelector('.magic-note-detail-header > :first-child')).toBe(screen.getByLabelText('笔记标题'))
...
describe('MagicNotesWorkspace canvas integration', () => {
  ...
  it('collapses the left index and AI independently without remounting the editor', async () => {
    ...
    expect(toggle.parentElement).toHaveClass('page-header__actions')
    const back = screen.getByRole('button', { name: '返回总览' })
    expect(back).toHaveTextContent('返回总览')
    expect(back.nextElementSibling).toBe(toggle)
    expect(toggle).toHaveTextContent('收起记录索引')
    const aiToggle = screen.getByRole('button', { name: '隐藏 AI 评论' })
    expect(toggle.nextElementSibling).toBe(aiToggle)
    expect(aiToggle).toHaveTextContent('隐藏 AI 评论')
    ...
    expect(toggle).toHaveTextContent('展开记录索引')
    ...
    expect(aiToggle).toHaveTextContent('显示 AI 评论')
  ...
  it('opens and closes the narrow index beside the back button and returns Escape focus', async () => {
    vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(700)
    render(<MagicNotesWorkspace onNotify={onNotify} />)
    await openNote()
    const toggle = screen.getByRole('button', { name: '展开记录索引' })
    expect(screen.getByRole('button', { name: '返回总览' }).nextElementSibling).toBe(toggle)
    expect(toggle).toHaveTextContent('展开记录索引')
    expect(document.querySelector('.magic-notes-index-pane')).toHaveAttribute('hidden')
    fireEvent.click(toggle)
    expect(toggle).toHaveAttribute('aria-expanded', 'true')
