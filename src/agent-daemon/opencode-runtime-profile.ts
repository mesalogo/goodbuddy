...
export function createOpenCodeLaunchProfile(input: {
  ...
  if (
    ...
    [...OPENCODE_REMOTE_RUNTIME_ENVIRONMENT_NAMES].sort().join("\0")
  ) {
    throw new Error(
  ...
  if (
    ...
    (!isStrictDescendant(modelBridge.bridgeDirectory, modelBridge.socketPath) ||
      !isStrictDescendant(
  ...
  const runtimeCommand =
    modelBridge === undefined
      ...
      : {
          executable: join(modelBridge.agentInstallationDirectory, "node"),
          args: [
            join(modelBridge.agentInstallationDirectory, "lib", "agent.cjs"),
            "model-bridge-helper",
            "--socket-path",
...
function normalizedAbsolutePath(value: string, label: string): string {
  if (value.includes("\0") || !isAbsolute(value) || resolve(value) !== value) {
...
function usableEnvironmentValue(value: string | undefined): value is string {
