...
- 新建、重命名、置顶、搜索、删除会话，以及从稳定的本地会话复制当前聊天内容到独立分支。
- 多轮上下文和会话级模型选择。
...
6. 创建会话分支前先保存当前稳定内容；分支使用新会话与消息 ID，显示来源徽标，不继承
   Task、队列、运行或成果归属。
...
- 左侧会话列表对关联 Task 显示行首展开按钮，展开后的 Task 子项左侧显示名称、右侧显示
  状态标记，父行不重复任务标签，展开层级只到 Task。
