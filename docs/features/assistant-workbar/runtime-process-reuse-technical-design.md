...
- Renderer 整包 metadata 加载是独立于进程复用的性能工作。现有实现改为列表摘要、
  打开会话时读取完整历史，不截断已保存消息；搜索、复制、续聊、后台活动和缓存释放
  规则见[会话列表读取与前端保留](./execution-history-storage.md#会话列表读取与前端保留)。
  不将这项后续修改的收益计入原进程复用对照结果。
