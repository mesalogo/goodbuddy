...
- 用户主动打开、关闭、排序和停靠可关闭的面板实例；任务中心和工作区作为基础工作入口默认
  存在且不可关闭，分别固定在第一、第二位，排序属性为不可重排。
- Task Center 是默认跟随当前项目的单实例 Task 索引。每个 Task 只关联一条 Conversation，
  一条 Conversation 可以关联多个 Task；Task Center 不复制会话内容，也不显示普通
  Conversation、Job、Run 或心跳事项。
- 当前能力、连接、数据和空状态可以动态变化。
- 能力不可用时，目录项或已打开面板显示原因、影响和可执行的配置或切换入口，不能只通过隐藏表示。
- 用户可以在设置中调整目录顺序；恢复默认布局恢复标准目录与默认打开面板，不强制打开全部能力。
- 浏览器、终端及其他声明为多实例的能力可以创建多个实例，每个实例拥有独立身份和范围绑定。

### 3.2 当前上下文只提供默认值

任务中心只有一个工作栏实例，任务列表固定跟随当前项目，不随当前 Conversation 筛选；顶部
统计跟随当前 Conversation。不提供全局或所有项目入口。其他可绑定
...
- 当前执行空间没有 Git 仓库时，工作区文件功能仍可使用，仅确认是 Git 仓库后显示 Git 工作区入口。
...
- 应用注册表分别声明实例策略、默认打开、必须存在、是否可关闭和是否允许排序。`single` 或
  `multiple` 只控制实例数量；`required` 控制布局修复，不能兼作关闭或排序策略。
- 在工作栏“+”目录中重复选择单实例应用时聚焦已有实例；选择浏览器、终端等多实例应用时
  每次创建新实例。选择已有 Tab 只激活该实例，保留其目标与历史。主侧栏应用中心不重复
  提供文件、终端、浏览器或资源监控入口。
- 支持方向键、Home、End、Enter、Space、可关闭面板的关闭命令和正确焦点恢复。不可关闭
  实例不渲染关闭按钮，也不响应关闭快捷命令。
- 徽标显示未解决数量、等待审批或失败状态，并同时提供文字或可访问名称。
- 用户调整目录顺序、打开实例、停靠位置和尺寸后持久化；关闭可关闭实例后能力仍可从目录
  重新打开。布局加载和恢复默认布局必须补回缺失的任务中心与工作区实例。
...
- 对话内容与输入区始终保留对称的左右安全边距，工作栏边界不得覆盖或裁切输入框、发送
  按钮及主工作区右侧边距。
- 唯一展开与收起开关固定在顶栏操作区、紧邻浅色/深色主题按钮之前。工作栏内部不重复关闭入口。开关图标中的
...
- 顶部显示当前会话消息总数和 Agent 累计回复时间；列表显示本项目任务的名称、关联 Conversation、
  所属项目、Ask/Execute、状态、最近进展、真实活动时间、累计执行时间和需要关注信息。
...
任务中心是应用级单例，默认存在且不可关闭。当前项目是唯一范围，标题明确显示项目；切换
项目刷新任务和审批，同项目切换会话只更新顶部统计。无项目时提示选择项目。后台状态
可以更新徽标和排序，但不能抢占用户当前工作。

回复时间按每轮请求区间累加，包含轮内工具和审批等待、不计轮间空闲；任务用时汇总历次执行
请求，不按父任务生命周期计算。缺失历史有提示，远程缺少可靠证据时不虚构时间。
项目任务标题右侧先显示“+”新建按钮，再显示筛选图标；状态筛选默认收起、默认全部。

主侧栏最近会话为关联 Task 提供轻量入口：父会话行只显示行首展开按钮，Task 子项使用
任务图标和本地化摘要，展开层级只到 Task；点击 Task 子项打开同一 Conversation 并定位。
新建定制任务使用共享 Modal，默认立即执行和当前 Conversation，可选本项目其他可用会话或
新会话，以及单次定时、每日或每周。内容必填，名称可留空从内容生成；工作模式使用关联
会话的执行时配置，未设置时使用项目默认值，不在创建表单另设模式。
...
工作区是默认存在、单实例且不可关闭的基础应用。它默认跟随当前项目；项目或目录不可用时
保留同一 Tab 并显示修复入口，不能通过关闭 Tab 隐藏范围错误。任务中心固定在第一位，工作区
固定在第二位，恢复布局时保持此顺序。
...
- 有界目录树和分页文本文件预览；文件超过单页传输大小时先显示已读取内容，并允许用户继续加载后续内容，不因文件总大小超过单页上限而拒绝预览。
- 当前选择、规范化路径和执行空间。
- 可选 Git 状态、Diff 和仓库信息。
- 显式打开、下载副本或在终端中打开。
- HTML 文件的源码与安全预览。

当前项目目录的文件浏览与 Git 交互遵循以下规则：

- FR-W1：顶部同一行放置“文件 / Git 工作区”视图切换和右侧刷新；非 Git 目录仍保留刷新。
  下一行只提供当前视图的工具：文件视图提供新建文件、新建目录，Git 视图提供分支、Fetch 和更改显示方式，
  不在 Git 视图重复目录导航或新建文件操作。操作使用带名称和提示的紧凑图标按钮。
  文件视图在根目录不显示孤立的文件夹图标或空路径行；进入子目录后提供“项目目录名 / 子目录”
  文字路径导航和返回父目录按钮。根目录文字可点击返回，提示显示完整根路径；返回根目录后焦点移至顶部刷新。
  长目录名单行省略，完整路径通过提示查看；多级路径在导航内部横向滚动，不撑宽工作栏。
  选中目录时在该目录创建，选中文件时在其父目录创建，未选择时在当前浏览目录创建。
  无可用目录时隐藏无关操作。
- 使用共享 `SegmentedControl` 切换“文件”和“Git 工作区”，默认显示文件；
  选择只在当前面板挂载期间保存在内存中，不持久化。未提交更改包含当前项目的全部
  Git 更改，不按 AI、用户或其他工具的修改来源筛选。
- FR-W2：只有已确认的 Git 仓库显示 Git 工作区入口。临时读取或连接错误保留同一项目
  已确认的 Git 可用性和内容，提供重试；切换项目不能沿用上一项目的确认结果。
- FR-W3：文件和目录行的更多按钮与左侧会话行一致，在悬停、键盘聚焦或菜单打开时显示。
  菜单复用会话操作的图标、间距和悬停样式，浮层限制在窗口内，不受文件列表滚动边界裁切；
  支持方向键、Home、End 和 Escape，Escape 或选定操作后焦点返回触发按钮，删除入口独立分隔。
  菜单提供重命名、移动、删除、属性、复制路径和本机默认应用打开；删除须明确确认对象
  与不可恢复性。操作失败保留输入与上下文。本机和远程均支持文件管理，系统默认打开仅本机可用。
  重命名允许仅修改同一目录项的名称大小写，不得覆盖另一个已存在的文件、目录或硬链接目录项。
  移动通过与创建项目相同的目录选择流程指定目标：本机打开系统目录选择器，远程浏览当前
  SSH Host 的目录。选择目录后保留原名，确认后执行；取消不移动文件。目标限于当前工作区，
  选择外部目录时在弹窗内说明限制，不切换项目或工作区范围。取消按钮使用共享次按钮样式，
  更多菜单中的删除图标和文字与其他菜单项一样左对齐。
- FR-W4：Git 分支标签打开可搜索的本地与远程分支选择器，支持创建和切换分支，以及显式
  Fetch。切换使用普通 Git switch，保留可携带的未提交修改；冲突时显示 Git 错误。
  不自动 force、reset、discard 或 stash，不因浏览分支而自动 Fetch。
  分支触发器复用共享模型选择按钮，带分支图标和展开箭头；Fetch 使用共享次按钮，二者在
  紧凑工作栏内保持同一行，长分支名省略并提供完整名称提示。
  分支选择面板浮在内容上方，展开时不挤占变更列表和提交历史；宽度不超过 360px，并限制在窗口内。
  搜索使用共享表单标签、输入框与主题样式，分支列表独立限高滚动，创建按钮以紧凑次操作放在列表下方。
  点击面板外或焦点移出时收起；Escape 收起面板并将焦点返回分支触发器。
- FR-W5：未提交更改支持列表和目录树切换；两种视图均可打开单文件 Diff。
- FR-W6：Git 工作区底部提供默认收起的提交历史区，独立滚动，可调整高度。
  历史标题默认停在面板底部，首次展开向上占用当前 Git 内容区的一半高度；收起后标题回到底部。
  上方变更列表和下方提交历史独立滚动；分隔条支持指针及键盘调整比例，底部位置保持不变。
  顶部刷新入口同时更新已展开的提交历史，历史区内不重复提供刷新按钮。
  按当前分支分页读取提交，展示摘要、作者、时间、短 OID 和 refs。选中提交显示该次修改文件，
  再选文件进入内联提交 Diff；逐层返回保留分支、分页、选择、滚动位置与焦点。
  提交 Diff 保留重命名前后路径和实际内容修改；后台工作区刷新不得使正在加载的历史或详情
  永久停留在忙碌状态。
  历史展开入口使用带方向箭头的紧凑列表行；提交行沿用文件列表的表面、悬停和焦点样式，
  摘要与元信息左对齐分行展示，长内容省略并提供完整提示，不撑宽侧栏。
- Git 错误和无更改提示只在 Git 工作区显示，不阻断文件浏览。
  切换项目时自动检测 Git 状态，无需手动刷新；检测结果和失败时保留的数据仅用于所属项目。
- 未提交更改以彩色 Git 字母标识状态：M 修改、A 新增、D 删除、R 重命名、C 复制、
  U 未跟踪或冲突、T 类型变化；冲突用危险色区别于未跟踪文件，同时保留原始状态。
- 点击更改项进入该文件 Diff，分别显示已暂存和未暂存差异，增删行使用对应状态色；
  删除文件也可查看。目录树中的文件仍打开内容预览。不再提供底部全仓库 Diff 入口。
- 文件预览保留在右侧工作区内；预览期间隐藏列表但保持目录树挂载，返回后保留已展开
  目录、滚动位置和选中行，并恢复触发行焦点。Diff 返回未提交更改列表时同样恢复
  滚动位置、选中行和焦点。
- 文件预览提供复制项目相对路径、刷新，以及仅本机项目可用的“使用默认应用打开”。
  刷新失败保留此前内容并显示错误；Markdown 默认渲染预览，可切换源码。
- Diff 提供刷新、复制项目相对路径和“查看当前文件”；删除项不显示查看当前文件入口。
- 手动刷新或任务结束更新 Git 状态时，同步重读根目录、当前浏览目录和已展开目录并去重，保留目录展开状态，
  刷新失败保留已有目录列表；重新展开目录时重读。目录读取错误在对应目录旁显示，
  可就地重试。正在查看的单文件 Diff 同时重读，旧请求不得覆盖新结果。
  无首次提交的仓库也应显示新增和暂存文件。
- 未提交更改首批显示 50 项，可继续分批加载其余变更，不能使后续文件尤其是已删除
  文件失去 Diff 入口。
- 系统默认应用和资源管理器打开按钮仅用于本机项目，SSH 项目不显示这两个按钮。

项目内目录导航属于 FR-W1；项目外目录选择、下载副本、终端打开及 HTML 安全预览仍属于后续能力。
本轮工作区交互不包含项目文件搜索，也不调整其他工作栏页签。
...
不能让多个工作栏 Tab 只是同一页面的重复控制入口。

浏览器能力在目录中稳定可发现，打开面板后允许用户：

- 在当前 Conversation 的隔离浏览器上下文中创建独立 Browser Tab。
- 选择当前 Conversation 或其他允许访问的 Conversation 作为实例目标。
- 查看状态、当前 URL、有界画面和错误。
...
工作栏实例不能在请求途中改变 Agent 的工具目标。用户在该实例执行前往、返回或刷新会有意
改变绑定到同一 Tab 的 Agent 后续操作。所有请求都进入 `BrowserService`；Renderer 不建立
第二套导航队列、抢占规则或恢复状态机。

用户可关闭 Agent 正在使用的标签页，无需先取消整轮请求。关闭只中止该页的浏览器操作，
其他标签页和请求的其他工具继续可用。后续快照等操作应提示标签页已关闭；Agent 显式导航
时打开专用新标签页并继续操作，不能自动接管用户的其他页面。
...
模型或后台浏览器活动可以更新对应实例的徽标，但不得无条件打开面板或切换用户当前面板。

浏览器多实例的跨进程身份、可见 viewport 所有权、关闭语义和 MCP 兼容方案以
...
资源监控是本机单实例面板，显示整机 CPU、内存、磁盘 IO 和 GoodBuddy 桌面进程摘要。面板
只在自身可见并处于活动状态时采样，数据保留在 Renderer 有界内存中，不进入运行记录或长期
存储。首期不提供远程主机、GPU、显存、网络吞吐、后台告警或任意进程查看。

指标缺失时保留面板入口并显示不可用原因。资源监控入口、采样频率、数据契约与验收由
[应用导航与系统工具入口](../application-tool-navigation/README.md)定义。
...
设置中心“平台功能”提供独立“远程项目”页签及开关，默认关闭。关闭时不显示
...
- 运行记录由 Main SQLite 持久化，Renderer 只保留当前内存视图。升级时迁移旧
  `localStorage` 记录；若旧缓存达到原数量、详情或总大小边界，页面持续说明更早记录或
  部分详情可能已经丢失。新记录不再按固定活动数或详情字符数丢弃，长列表只做可继续展开的
  分批渲染。
...
- 工作区、成果、浏览器和监督的既有或扩展服务，以及有界的本机资源快照。
...
本节为待实施设计。用户需要在对话顶部查看 Agent 本轮列出的工作项及完成情况，避免在
工具记录中反复查找。这里的“执行清单”由 Runtime 更新，术语边界见
[Task 与 Job 统一领域模型](../task-and-job/task-and-job-model.md#17-runtime-执行清单)。

- **FR-CL1**：接入 OpenCode（OC）的 `todowrite` 与 Continue（CN）的 `Checklist`，
  范围包括本地及远程 GoodBuddy Agent 上运行这两个 Runtime。远程 CN 的部署和正常请求
  路径也属于交付范围，当前可用程度见[进度](./progress.md#2026-09-15-runtime-执行清单设计未实现)。
- **FR-CL2**：顶部展示本轮最新有效清单，更新按整份替换处理，支持明确清空。每轮请求、
  会话、项目、执行空间与 Runtime 的清单分别归属；普通聊天续轮不能沿用旧清单冒充本轮进度。
- **FR-CL3**：清单只读，用户不能手动勾选。请求完成、失败或取消保留清单实际状态，
  不自动完成工作项；原生工具调用继续进入消息与活动记录。
- **FR-CL4**：重新打开会话可恢复已保存的清单及请求结果，远程重连遵循已有 Agent 恢复
  流程，不重新发送模型或工具请求。没有原生清单的请求不显示空占位。

ACP 仅是传输及远端协议，不分类为 Runtime。DeepSeek Harness（DS）真实默认环境无
todo/plan，本次不为其增加工具。也不增加 plan 工作模式、计划文档、计划编辑器或定时任务
能力；只有格式已确认的原生 plan entries 可以适配清单。

#### 顶部布局

执行清单位于聊天内容区顶部、已有 Task 上下文栏下方、消息流上方；没有 Task 时仍可独立
显示。它不占用窗口标题栏操作区，也不替换 Task 摘要、任务索引或右侧 Runtime 栏目。
布局遵循 [UI-DESIGN](../../../UI-DESIGN.md#610-应用顶栏与全局操作)。

默认折叠为一行“执行清单 · 已完成 m/n”，有进行中条目时附简短摘要；展开后按原生顺序
显示内容、状态及原生提供的优先级。行内状态使用图标和“待处理 / 进行中 / 已完成 / 已取消”文字，
不用可勾选 Checkbox，也不根据位置推测优先级或进行中状态。请求结束后的摘要附真实
请求结果，例如“已取消”，不改变条目状态。后台更新不强制展开、不抢焦点。

展开按钮提供 `aria-expanded` 和 `aria-controls`，支持 Enter/Space，折叠后焦点留在
按钮；实时区域只播报摘要变化。复用全局颜色、间距和字体令牌，浅深主题均可读。窄窗口
摘要截短、展开内容换行并有界滚动，保留完成计数、请求结果与输入区；200% 缩放时仍可
操作。没有清单或收到有效空清单时整行隐藏，历史内容仍按消息归属查看。

#### 场景验收

| 场景 | Given / When / Then | 需求 |
| --- | --- | --- |
| US-CL1，连续更新 | OC/CN 本轮产生清单并修改、重排或删除条目；顶部显示最新完整内容，工具活动仍可查 | FR-CL1–FR-CL3 |
| US-CL2，请求隔离 | 同会话开始下一请求、切换 Runtime 或并行使用另一会话；旧轮迟到事件不覆盖当前清单，历史按原归属保留 | FR-CL2、FR-CL4 |
| US-CL3，失败与取消 | CN 更新失败或请求被取消；失败候选不替换上次有效清单，取消不把未完成项勾完；有效空清单清除顶部 | FR-CL2、FR-CL3 |
| US-CL4，本地与远程恢复 | 重新打开本地会话或重连共享 Linux Host 上的 OC/CN 请求；已有记录按原请求恢复，重复事件不重复展示，不重新调用模型或工具 | FR-CL1、FR-CL4 |
| US-CL5，无清单与无障碍 | DS 默认请求或普通 Markdown/list 工具输出没有已确认清单；顶部不出现伪清单；有清单时键盘、窄窗口及浅深主题可用 | FR-CL1–FR-CL3 |

行为规则见 [Runtime 交互边界](./runtime-interactions.md#runtime-执行清单)，接入与验证见
[技术设计](./runtime-checklist-technical-design.md)。
...
2026-09-14 调研确认：多个项目可能各自保留一个重型 Runtime，状态或原生清单读取也会
影响其启动与保留。用户需要多项目/多会话自然并行工作，而不是为避免内存增长关闭工具、
子代理或删除历史。以下为新需求，**不表示 Desktop 0.13.2 / Agent 0.11.25 已实现**。

- **FR-RR1**：支持原生多会话的 Runtime，在同一账号和兼容启动配置下跨项目复用重型
  进程；不得仅因项目目录或会话不同重复启动。等价的默认和显式选择应得到相同复用结果。
  不兼容的 Runtime、Host、模型连接或插件配置可以分开，不要求全产品只有一个进程。
- **FR-RR2**：仅查看被动状态不启动重型 Runtime。显式连接测试、原生清单和执行请求
  复用兼容的已启动实例；尚未握手时不能显示为已经验证连接。
- **FR-RR3**：共享进程仍保持各会话的目录、历史、模型用量、工具、权限和业务问答归属。
  同一会话顺序执行，不同会话并行；Ask 保持只读，Execute 不增加第二层审批。
- **FR-RR4**：请求结束、会话释放与进程退出分别处理。取消一个普通请求、关闭工作栏
  或释放一个会话不得误停其他健康会话；无会话的空闲重型实例及时释放。正常续聊保留
  原生上下文，应用退出、配置替换和真实进程故障仍有明确的终态与有界回收。
- **FR-RR5**：托管 SSH 独立实现并验证复用，保留 Agent 对活动 Prompt 的所有权、
  断线续接、事件去重和模型调用结果不确定性规则。不能以本机通过代替远端交付。
- **FR-RR6**：对不具备原生多会话 Host 的 Continue 保留正确的请求隔离；直连模型不
  新增独立模型进程。优化应有前后性能证据，不以删历史、禁用工具、全局串行化或
  推迟正常用户操作来换取进程/内存指标。
...
| 场景 | Given / When / Then |
| --- | --- |
| US-RR1，首次与热启动 | 同一有效配置的两个项目，分别查看状态、打开清单并开始四个对话；状态不冷启动，后续共用一个可多会话的重型 Runtime |
| US-RR2，并行与取消 | 两个项目有同名不同内容文件，四个会话进行真实开发；目录及成果正确，取消其中一个后其余三个继续完成 |
| US-RR3，续聊与释放 | 一轮任务完成后继续原会话，再释放其中一个；原生上下文可续用、其他会话不受影响，数据库完整历史仍可查看 |
| US-RR4，边界与远程 | 切换不兼容配置、查看 Continue/直连状态或中断 SSH；不串配置、不伪造多会话能力，Agent 已接受工作按现有归属恢复 |

资源状态规则由 [Runtime 交互边界](./runtime-interactions.md#runtime-资源生命周期)
定义；锁定版本研究、实现选择与逐项验收由
[进程复用技术设计](./runtime-process-reuse-technical-design.md)定义。
...
- 审批在所属任务或 Runtime 中持续可见；任务中心只为未关联任务的审批保留按需显示的处理
  区域，具体规则见 [Task Center 交互](../task-and-job/task-center-prd.md#5-交互)。
- 智能心跳菜单入口承接完整配置和范围后，再从任务中心移除重复表单；不得移除任务中心本身。

### 阶段 1：成果与 Runtime 可观测性

- 本机 CPU、内存、磁盘 IO 和 GoodBuddy 桌面进程摘要的按需资源监控。
- 通用 Artifact Event。
- HTML 工作区和成果的静态即时预览。
...
- [Task Center PRD](../task-and-job/task-center-prd.md) 定义单实例、项目跟随和显式范围切换。
