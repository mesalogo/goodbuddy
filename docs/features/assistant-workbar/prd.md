...
任务中心只有一个工作栏实例，默认跟随当前项目，不跟随当前 Conversation。用户可以在同一
实例内显式切换到所有项目，但不能为不同项目打开多个任务中心 Tab。其他可绑定
...
- 在工作栏“+”目录中重复选择单实例应用时聚焦已有实例；选择浏览器、终端等多实例应用时
  每次创建新实例。主侧栏系统工具快捷入口按当前上下文聚焦匹配实例，不能因连续点击无意
  创建重复实例。
...
- 显示名称、关联 Conversation、所属项目（无项目时标注“未绑定项目”）、Ask/Execute、状态、最近进展、
  真实活动时间和需要关注信息。
...
任务中心是应用级单例，默认存在且不可关闭。它的数据范围默认跟随当前项目，并在标题或范围
控件中明确显示项目；用户可以切换到“所有项目”，该选择只改变同一实例的列表
范围，不创建第二个任务中心 Tab。切换回“跟随当前项目”后，项目变化立即刷新列表。后台状态
可以更新徽标和排序，但不能抢占用户当前工作。

当前实现已按活动项目过滤 Scheduled Task 和审批，并提供“当前项目”和“所有项目”
范围控件。
