...
- “监督者 > 设置”是自动监督配置、报告、建议和运行历史的权威位置；内部继续沿用 heartbeat 标识。
...
1. 完整自动监督配置统一到“监督者 > 设置”，打开后直接展示表单。
...
| 操作 | 权威入口 | 其他位置 |
| --- | --- | --- |
| 创建或编辑自动监督配置 | 监督者 > 设置（默认显示） | 不复制表单 |
| 选择 Global / Project 范围 | 监督者 > 设置 > 自动监督 | 不依赖当前项目推断 |
| 暂停、恢复、立即运行或删除 | 监督者 > 设置 > 自动监督 | 不复制操作 |
| 查看报告与待处理建议 | 监督者 > 设置 | 保留既有结果 |
| 查看运行历史和失败 | 监督者 > 活动 | 按同一次执行展示心跳报告和下游监督状态；设置 > 报告与记录保留报告本身的历史 |
...
监督者设置保留四个面板，默认打开自动监督：

- **自动监督**：唯一完整配置入口，界面规则见[监督者 UI 设计](../conversation-supervision/ui-design.md#自动监督设置)。
- **运行概览**：状态、成功率、记忆、洞察和行动统计。
- **待处理建议**：记忆建议和行动建议。
- **报告与记录**：回顾报告与 Run 审计。
