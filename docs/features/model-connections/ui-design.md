...
它们收纳在默认折叠的“高级设置”中，不新增页签或弹窗。只要其中一项无效，保存时自动
展开该区域并保留对应字段错误。每个编辑器使用可纵向调整大小的等宽文本域，默认值为
`{}`。
...
- 新连接从两个空对象开始。
- 保存前校验失败时，页头错误和对应字段错误都显示连接名称；模型接口 URL 同时标记
  `aria-invalid` 并关联就地错误，避免只显示无法定位的“Invalid URL”。
- LLM 模型连接的分类标题和模型类型分段页签在内容滚动时共同粘附在内容区顶部；左侧连接
  列表仍可独立滚动。
- 每个已保存或正在编辑的 LLM 连接提供“复制连接”。复制会生成新的连接名称和 ID，保留
  地址、模型、协议、请求定制、认证方式及当前草稿 API Key。已保存的 API Key 通过 Main
  进程的加密凭据复制路径保存，不回传到 Renderer；原连接和默认连接不改变。
