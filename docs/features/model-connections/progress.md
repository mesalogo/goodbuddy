...
## 2026-09-20：模型设置可用性修复

- 模型接口 URL 保存失败时按连接名称显示错误，URL 字段同步提供 `aria-invalid` 和就地
  错误；共享契约同时拒绝空值、非 HTTP/HTTPS 地址和超长地址。
- 模型连接分类标题与 LLM、向量、重排、语音分段页签在模型内容滚动时共同固定；自定义
  Header/Body 默认收纳在折叠的“高级设置”中，无效时自动展开。
- LLM 连接新增复制入口。复制保留当前草稿、请求定制和认证设置；已保存 API Key 由 Main
  进程从加密凭据复制，不进入 Renderer 或错误信息。
- 已新增 SettingsPanel 与 RuntimeSettingsStore 回归覆盖；完整验证结果见后续条目。
