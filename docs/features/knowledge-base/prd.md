...
| 相关设计 | [本地文本向量模型与连接设计](./local-embedding-technical-design.md) |

## 1. 背景

本文负责本地检索与分块增强。当前工作区另已接入外部实例、绑定和统一检索，其产品范围见
[外部知识库 PRD](./external-knowledge-prd.md)，实现与待验收状态见[实施进度](./progress.md)。
外部绑定不进入下述本地分块、向量索引和图谱维护流程。
...
3. 保留“模型按需检索”，并新增“每次先检索”模式。后者必须由 Main 进程
   ...
   `knowledge_search` 的 `limit` 与现有检索引擎对齐，接受 1–100，省略时返回最多 6 条；
   调用方可请求超过 8 条。授权范围、检索排序和工具结果字节预算继续生效，因此实际
   条数可能少于请求值。
4. 知识库新建后不默认启用全部已有知识库；对话中的范围继续由用户显式选择。
5. 向量服务不可用时保留全文与中文检索，但必须返回明确降级状态。这是可见的检索通道
   降级，不得自动切换内置连接、用户添加的 OpenAI 兼容连接或其他向量模型。
...
9. 向量索引兼容性同时校验 Provider、Model、维度和 Provider Fingerprint。
   ...
   数据路径定义以[本地文本向量模型与连接设计](./local-embedding-technical-design.md)
   为准。
...
14. Ask 的运行时边界保持只读。知识库内容始终被标记为不可信证据，
    不能成为系统指令。
15. 向量连接是 GoodBuddy Main 管理的知识服务依赖，不是 Agent Runtime 模型。它不进入
    直连模型、OpenCode、Continue、DeepSeek Harness、远程 Runtime 或项目 Runtime 的
    模型选择、配置和凭据边界。
16. Runtime 只能获得 Main 检索后选出的有界文本证据、来源引用和脱敏诊断，不能获得
    Embedding Provider、连接 Profile、Endpoint、API Key、模型目录、Tokenizer、原始向量
    或索引 Fingerprint。
...
- 把向量连接用作 LLM、Agent Runtime 模型或 Runtime 模型网关。
- 让 Runtime 直接调用 Embeddings Endpoint、内置推理进程或向量数据库。
- 未经独立设计就把知识库向量连接扩展到记忆、自动化、项目文件搜索或其他应用服务。
...
“检索测试”是当前知识库的高频验证操作，通过知识库标题区和“高级设置 →
问答效果”打开同一个独立工作台，不新增第五个一级页签。工作台首次打开只突出真实
问题输入和测试操作；候选倍数、权重、阈值、重排和上下文预算默认折叠。

首次使用流程固定为“创建知识库 → 添加资料 → 等待可检索 → 用于当前对话”。创建表单
默认只展示名称和描述；存储方式与知识图谱属于折叠的高级设置。没有知识库时，唯一主操作
位于首次使用空状态中。

全局向量模型仍在“设置 → 模型连接 → 向量模型”中配置。GoodBuddy 内置连接和用户添加的
OpenAI 兼容连接使用同一连接列表；其界面、数据路径及切换语义以
[本地文本向量模型与连接设计](./local-embedding-technical-design.md)
为准，知识库页面只显示当前模型、索引兼容性、覆盖率和重建操作。“模型连接”是设置入口
名称，不表示向量连接可以分配给 Agent Runtime；项目 Runtime 和 Agent Runtime 的模型
选择中不显示向量连接。

知识检索与 Runtime 的关系固定为：

```text
用户问题
→ Main / KnowledgeService 使用当前向量连接检索
→ Main 生成有界文本证据、引用和检索诊断
→ 当前 Runtime 仅消费这些不可信检索结果
```

`always` 模式由 Main 在启动 Runtime 前完成检索；`auto` 模式中的 `knowledge_search`
也只允许 Runtime 调用 Main 的有界知识工具。两种模式都不把向量连接、凭据、模型文件、
Tokenizer、原始向量或索引访问能力传入 Runtime。切换 Runtime 不改变当前向量连接。
...
知识库 ID 选择按 Conversation 持久化。新对话和未保存过知识范围的旧对话默认不选择
任何知识库；切换对话恢复各自范围，不得在应用启动时自动勾选全部知识库。新建知识库只
加入当前 Conversation，不重新启用用户已取消的其他知识库。
...
图谱通道对中文自然语言问题生成有界 CJK 二元词和拉丁关键词，在实体名称、
别名和类型中确定种子，再沿具有有效文档证据的关系执行有界多跳扩展。图谱探索页
搜索到实体时保留其一跳邻居和关系上下文，并继续允许从实体或关系打开原始证据。
...
`indexedDocumentCount` 只统计状态为 `ready` 的文档；`queued`、`parsing` 和
`indexing` 计入处理中，`failed` 单独计入需处理。文档行持续显示全文、向量和图谱通道
状态。已有可用分块的失败文档仍允许查看分块，并提供重新处理。

文档行可通过受控 `knowledgeBaseId + documentId` 打开数据库中保存的原始来源。
Renderer 不得提交任意路径。移除来源必须先说明将删除的文档、索引、图谱证据及是否包含
应用托管副本，用户确认后才执行。
...
| 图谱模型空响应、超时或瞬时服务错误 | 按不超过约 8,192 个估算 Token 组织独立抽取批次，每批最多尝试两次；部分批次成功时发布已验证结果并在任务中显示具体警告；所有批次失败时图谱任务失败，但新文档的全文索引仍可发布，已有文档的显式重建继续保留上一版可用索引 |
...
- 内置连接和用户添加的 OpenAI 兼容连接之间不会自动切换；用户连接的实际数据去向持续
  可见，内置连接不重复展示固定本地路径。
- 项目 Runtime 和 Agent Runtime 的模型选择不显示向量连接。
- Runtime 只获得有界文本证据、引用和脱敏诊断，不获得向量 Provider、凭据或原始向量。
- 切换直连模型、OpenCode、Continue、DeepSeek Harness 或远程 Runtime 不改变当前向量
  连接。
