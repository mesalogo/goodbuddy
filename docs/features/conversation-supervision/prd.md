...
本 PRD 继续定义会话监督能力。统一应用“监督者”提供工作回顾、故事线图谱、自动监督、活动记录与设置五个页签；实际能力及执行记录要求见[监督者应用产品设计](./supervisor-prd.md)。本文其余执行观察与介入目标仍按各节范围实施。
...
监督应用启用后，助手工作栏提供监督栏目；关闭时不挂载或查询监督面板，见[应用启停与执行](./logic-design.md#应用启停与执行)。栏目默认
