...
本功能补齐直连模型缺少的基础 Agent 能力：

1. 一组工作区搜索、分页读取和补丁编辑接口。
2. 一个按平台适配的进程执行接口。
3. 一个单层、继承父请求范围的编程 Subagent 接口。
...
- 当前工作区默认工作目录，以及当前用户可访问的本机工作目录选择。
...
- 安装包按目标平台和架构携带固定版本 ripgrep，并将其提供给工作区搜索和命令 PATH。
...
- 默认目录是父请求当前工作区。
- 可选工作目录支持绝对路径、相对工作区路径及指向工作区外目录的路径或符号链接，
  不额外限制在工作区内；目标必须是实际存在且当前用户可访问的目录。
- 本机执行只能发生在当前桌面执行空间。
...
- 直连文本模型的普通问答、工具轮次和上下文摘要遇到瞬时网络错误或单次请求超时时，
  在尚未显示文本或推理内容的前提下最多自动重试 3 次。
- 重试使用递增等待时间并显示当前次数；用户取消必须立即终止等待和后续请求。
- 一旦本次模型调用已经显示文本或推理内容，不自动重放该调用，避免重复回答。
- 非网络类 Provider 错误、协议错误和用户取消不进入自动重试。

### FR-12 高效工作区工具

- `workspace_rg` 同时支持内容搜索和文件列表，遵循项目忽略规则，并以
  `path:line:column:text` 紧凑格式返回匹配。
- `workspace_read_text` 接受起始行和行数，大文件通过续读完成，不因超过旧的整文件上限而
...
- `workspace_rg` 使用固定参数调用随包 ripgrep，不向模型开放 `--pre` 或任意命令参数。
- 工作区工具不限制文件总大小、补丁总大小、搜索表达式长度、glob 数量或模型请求的结果
  数量。单次返回按上下文容量截断或分页；读取结果提供续读位置，搜索结果提示模型缩小查询。
