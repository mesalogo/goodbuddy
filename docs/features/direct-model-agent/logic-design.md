...
- 超时和取消结果包含已取得输出的预览；超出预览的内容按 FR-7 继续保留，直到所属会话释放。
...
| --- | --- |
| 未提供 | 当前工作区根目录 |
| `.` 或相对路径 | 以当前工作区为基准解析，可指向工作区外目录 |
| 绝对路径 | 使用指定目录 |
| `..` 或指向工作区外的符号链接 | 解析到实际目标目录，按当前用户权限执行 |
| 不存在或不是目录 | 拒绝 |
| 当前工作区身份在验证期间变化 | 拒绝本次调用 |

工作区提供默认目录和相对路径基准，不是 Execute 命令的文件系统边界。单次命令选择其他
工作目录不会修改父请求绑定的项目或工作区，也不会影响后续未指定 `cwd` 的命令。
...
- FR-5 / FR-7 / US-C1：Execute 子级使用父请求的浏览器标签页，不另开标签页或改变所有权；
  子级结束后父模型仍可继续操作该页，父请求继续负责其使用租约。
