...
当前项目的 Task。用户可以在同一 Tab 内切换到“所有项目”，不能为每个项目创建
独立 Task Center 实例。
...
- `所有项目`：显示当前用户可见的项目 Task 和没有 `projectId` 的历史 Task，后者标注“未绑定项目”。

没有活动项目时，“当前项目”显示明确空状态，并允许切换到“所有项目”；系统不
自动改变用户已经选择的范围。范围模式作为工作栏公开偏好持久化，但“当前项目”模式不保存
具体项目 ID；重启后继续跟随届时的活动项目。旧布局保存的 `global` 范围在 UI 恢复时映射为
`all-projects`，后续保存使用新值；不修改任务数据或后端项目绑定规则。

## 5. 交互

- 点击条目打开关联 Conversation，并定位到该 Task。
- 状态筛选仅保留进行中、暂停、已结束，默认选择进行中。
...
- [x] 仅提供“当前项目”和“所有项目”；后者保留无项目 Task，并标注“未绑定项目”。
