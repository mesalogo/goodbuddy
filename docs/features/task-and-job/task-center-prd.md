...
- FR-TC4：项目任务标题右侧放置“+”新建按钮，不再提供独立筛选图标。
  项目任务列表默认收起，标题与数量、新建入口保持可见；点击标题切换展开状态。
  标题旁同时显示当前项目顶层任务中处于 `running` 状态的数量（如“2 运行中”），不受筛选或收起影响；为零时省略。
  展开后直接显示全部、进行中、暂停、已结束状态选项，初始选中进行中；收起不清除已选条件。
...
- [x] FR-TC4：标题右侧仅保留新建；展开列表直接显示状态选项，默认进行中，收起后保留所选条件。
