...
当前状态以 [2026-09-19 实现与验证](#2026-09-19-实现与验证)、[附件操作布局复验](#2026-09-19-附件操作布局复验)、[全局浮层复验](#2026-09-19-全局浮层复验)及[空正文与草稿恢复修正](#2026-09-19-空正文与草稿恢复修正)为准。
...
修复前源码在 Electron 中复现：“测试解析”的 `AnchoredMenu` 层级为 129，低于设置
遮罩的 130；通知层级为 75，且仍在被设为 `inert` 的应用壳层内。两者的鼠标命中均失败。
共享修复及后续约束见[复发缺陷规则](../../quality/recurring-defects.md#modal-内的菜单与通知被遮挡或隔离)。

- `FloatingPortal.test.tsx` 覆盖 Modal 切换时保持同一通知宿主和内容、焦点循环及菜单归属。
- `overlay-layering.electron.test.ts` 使用生产组件与受控 IPC，验证浅/深主题下的
  1280×800、960×720、720×640、640×420；包含通知在 Modal 前/后出现、点击关闭、
  Tab/Shift+Tab/Escape、解析场景菜单与二级结果、应用中心、本机推理监控和原生 `dialog`。
- 完整 App 使用当前 Main、Preload、Renderer 的独立构建及空白 profile，从侧栏设置入口
  修改最大页数为 99，确认生产读取值及成功通知；原生鼠标点击“测试解析”菜单后，生产 IPC
  解析本地 TXT，结果展示 70 字符，Escape 逐层关闭并恢复焦点。仅文件选择器返回合成文件；
  未替换解析、保存或通知。模型及 OCR 服务调用均为 0。
- 已通过图片读取工具逐张审阅 fixture 的 16 张 `light/dark-宽度-menu/notification.png`，
  以及完整 App 的保存通知、测试菜单和解析结果三张 PNG；文案、主题、菜单与通知层级正常。

临时证据位于批准目录下的 `overlay-baseline-0919`、`overlay-layering-0919` 和
`overlay-full-app-0919-r2`。完整 App 首轮因驱动选择器错误失败，修正后通过；仍记录到
4 条既有 data-font CSP 警告。本轮实际 UI 环境为 Windows x64、Electron 43.2.0，
未验证 macOS、Linux 或辅助技术的实际语音播报。
本轮临时 profile、基线缓存和独立构建已清理，保留脚本、脱敏报告、源码/产物哈希及截图。

最终 `npm test`：403 个文件通过、9 个跳过，4,793 项通过、67 项跳过，耗时 909.69 秒。
`npm run typecheck`、`npm run lint` 及隔离目录的 Electron Vite 生产构建通过；
相关文档的 48 个相对文件链接有效，`git diff --check` 通过。

## 2026-09-19 空正文与草稿恢复修正

对应 L-I3、L-H7、L-H8。修复前已复现：无识别文字、有效插图与缺图警告并存时，
警告被当作正文，显式提取后请求没有原图；队列恢复等待期间输入的文字也会被旧草稿覆盖。
实现和判定归属见[生产实现说明](./implementation.md)，未增加持久状态或修改数据 schema。

- 解析、结果完整程度和 HTTP Provider 共用提取正文判定。8 个解析/存储/共享 UI 测试文件
  203 项通过，其中 loopback HTTP 回归覆盖缺图警告、正文存在与否及保存后的 `images-only`。
- 生产 `App` 的 6 项回归通过，覆盖空草稿、已有草稿、恢复期间编辑、清空、失败及切换会话。
  IPC 响应由测试控制完成时机，不能称为完整桌面到 Main 的新增端到端验收。
- 当前源码在隔离 Electron 中通过真实 `nativeImage`、HTTP 客户端、附件 SQLite/文件存储、
  `reparseDraft` 与 `enrichRequest` 复验：仅图片及图片加缺图警告均拒绝文字切换，
  草稿仍引用原附件，发往 Runtime 的请求保留一张图片；真实正文控制组仍切为文字发送。
  三次请求均发往本机合成 OCR 服务，外部 OCR/文本/图像 Provider 请求为 0。
- 本轮 OCR 判断和草稿合并仍位于 Desktop，共用同一 Main 请求构造与 Renderer 草稿机制；
  没有远端 OCR 分支、Agent 协议或模型桥变更，不据此扩大远端视觉模型验收结论。

验证用的临时 fixture、Electron profile 和数据库已清理。首次 OCR 探针因临时目录的模块
解析失败，调整启动器的依赖路径后通过；新会话切换测试最初遗漏全局队列读取，修正
fixture 后通过。上述均为验证工具问题，不修改产品行为来满足错误 fixture。

合并画布与 Continue 修正后的 `npm test` 完整结束：405 个文件通过、9 个跳过，
4,826 项通过、67 项跳过，耗时 982.48 秒，无失败。最终 `npm run typecheck`、
`npm run lint` 及助手数据库 101 项单独复验均通过。会话列表测试更新为无 100 条截断的
既定行为，共享高对比度测试检查全部 forced-colors 块；没有为旧断言恢复产品限制。
隐藏 Electron 窗口的双帧等待实测令布局场景耗时约 63 秒，因此只把测试子进程期限从
60 秒改为 90 秒，并对自有临时目录增加有界删除重试；完整布局断言未减少。
