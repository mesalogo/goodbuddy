...
HTTP OCR、显式图片提取及结果完整程度共用 `hasExtractedDocumentText`；图片引用与生成的
缺图警告只作预览/诊断，不算正文。无正文的显式提取失败并保留原草稿；有有效插图的文档
结果仍可保存为仅图片，不因缺图警告改判为可发送文本。
...
再移除队列项，文字用空行追加，超限保持原队列。Renderer 请求恢复时不传入旧草稿文本，
收到队列正文后通过原会话的函数式状态更新追加到最新草稿，保留等待期间的编辑和清空。
