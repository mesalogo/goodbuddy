...
应用中心、可选应用常驻及排序、启用联动、共同设置页和本机推理已接入生产源码；本机资源监控的
...
| `ApplicationSettingsView` | 位于 `ApplicationCenter.tsx`，应用中心、应用页面、全局设置共用的应用配置表单 |
...
策略。打开前检查确认的启用值；未常驻但已启用的应用不附带常驻写入。本机推理从常驻入口、菜单及管理 Modal 均经 `setView('local-inference')` 打开独立 Modal，保留底层工作区与设置草稿；其他应用继续页面导航。
`ApplicationMenu` 按知识库、智能心跳及可选顺序派生清单，仅按启用过滤，不读取常驻或打开历史。应用行先关闭菜单再导航；底部“管理应用”关闭菜单、恢复锚点焦点并打开既有 `ApplicationCenter` Modal。
中心和应用页的设置操作仅接受 `EditableApplicationId`，全局设置旧入口定位同一个共同页。知识库和智能心跳的管理行仅提供“打开”，原工作区实际配置保留，不添加通用空设置页。
...
type ApplicationNavigationSettings = {
  order: EditableApplicationId[]
  pinned: Record<EditableApplicationId, boolean>
}
```

`applicationNavigation` 仅保存可选应用顺序和常驻布尔值。默认顺序为
`magic-notes`、`local-inference`，两项常驻默认 `true`。Zod 验证顺序恰好包含
两个可选 ID 各一次，`pinned` 仅接受这两个键。知识库和智能心跳不进入导航 schema；Renderer 将它们固定置于可选应用之前，再按启用和常驻过滤可选项。取消常驻或关闭可选应用不删顺序。
按[排序规则](./logic-design.md#3-主导航组合规则)提交完整顺序；不持久化可推导的可见 ID 列表。
首期不预建插件字段，也不为未发布的旧隐藏 ID 草案增加迁移读取器。

### 4.2 与现有开关组合

可选应用侧栏入口可见条件如下，知识库和智能心跳固定显示：

```ts
const visible = featureEnabled && applicationNavigation.pinned[id]
```

`magicNotesEnabled` 保留为笔记启用的唯一权威键，`localInferenceEnabled` 为本机推理应用启用键，默认 `true`，不在导航配置再复制 enabled map。心跳没有应用级启用字段，单条计划 enabled 的业务含义和存储不变。关闭可选应用时保留常驻控件和恢复入口。
...
存储版本 12 补充可选应用导航和启用字段，仅为缺失笔记字段补新默认值；保留已有
`false`、单选值和无关设置，不迁移或清理笔记、知识及会话业务数据。
版本 12 属于未发布的分支格式，原草案 `knowledgeEnabled`、`heartbeatEnabled` 已移除；不为它们或旧三项／四项导航草案增加兼容读取器或迁移。
...
字段。keep-alive 页挂载不代表应用仍启用；禁用后在原位置渲染不可用状态和共同设置恢复入口，
保留页面草稿及业务数据，阻止新的页面操作。重新启用不触发导航或业务重放。

`ApplicationAvailability` 隐藏并设置业务内容为 inert，保留挂载的页面实例；恢复入口由
`ApplicationSettingsNavigation` Context 定位同一中心表单。笔记页面直接消费确认的评论配置，
不因修改评论方式而重建编辑器。
...
- US-F3／US-F5：三个入口均打开本机推理 Modal 并保留底层工作区；覆盖嵌套确认的焦点循环、逐层 Escape、提交期间禁止关闭与重复执行、关闭后的入口及后备焦点恢复，以及停用内容后的键盘焦点。
- US-B7：常驻、取消常驻、键盘及拖动排序成功后同步，重新开启及重启保留原偏好和顺序。
- US-B8：知识库和智能心跳始终启用且位置稳定，管理行只有打开操作，心跳计划 enabled 不变；可选应用关闭后的不可用页保留草稿，重新开启不自动导航或重放。
- US-B9：应用中心与笔记页定位同一笔记表单，启用、常驻、待办数量、评论方式及形式共用配置；全局设置无笔记入口。
...
- 顺序包含两个可选 ID 各一次；拒绝知识库、智能心跳、重复、缺失、未知 ID 及无效常驻字段；不为未发布的两个旧启用键添加迁移。
- 未配置布尔项默认开启；已保存的 `false` 和评论单选值经迁移及重启保留。
...
以下为实施阶段划分，应用中心及本机推理已接入源码，验证状态以进度记录为准。
...
4. 迁移魔法笔记共同设置，保留知识库、智能心跳固定入口及原工作区配置、单条计划 enabled，验证可选应用关闭和重新开启。
