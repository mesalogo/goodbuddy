...
3. 以 Ask 只读问答和 Execute 完整授权两种模式完成工作；计划审查是 Ask 中的交互方式，不是第三种权限模式。
4. 在应用级助手工作栏中持续提供任务中心、监督、Runtime、终端、进程、工作区、浏览器和成果。
...
- 所有窗口宽度下右侧栏都停靠显示并推动主工作区水平回流，不覆盖或遮罩主内容。
- 排除最左侧主导航后，主工作区与右侧栏使用相同的 `300px` 最小宽度，并按用户拖动后的
  比例分配可用宽度；任一侧到达最小值后停止拖动。
- 主工作区中的消息流和输入区保留对称的左右安全边距，右侧栏不得覆盖或裁切其右边缘。
- 唯一展开与收起开关固定在窗口关闭按钮正下方，右侧栏内部不重复关闭入口；当前会话没有
  Task 时不保留空的 Task 上下文栏。
- 右侧栏在对话、知识库和活动视图之间保持状态。
- 知识图谱实体详情复用同一右栏容器，不再维护独立布局。
- 八个标准能力固定可达；能力、连接和数据状态可以变化，但应用不按当前上下文自动隐藏入口。
...
- 本次请求使用的附件、知识库、截图和剪贴板由 Composer 展示与管理。
- 授权目录、浏览器和工作区内容由对应工作栏能力展示，不创建重复的上下文栏目。
- 历史执行上下文随关联 Task 查看；记忆功能实现后归入任务中心。
- 不显示或持久化用户未主动选择的桌面内容。
...
- 智能心跳长期按分区回顾信息并提出记忆候选或唤起建议，但协同规则、唤起条件、生命周期、
  页面以及与任务的关系尚未设计，不能提前实现平行的记忆类型或页面。
...
4. 远程委派支持用户配置的 HTTP 和 HTTPS 地址，不按主机位置增加额外限制。
...
- 会话与运行记录已从 `localStorage` 迁移到主进程数据库；旧运行记录可能触及历史上限时
  保留可见的不完整提示。
- 拆分共享契约和 IPC 注册。
...
- 应用级助手工作栏壳层。
- 固定任务中心、监督、Runtime、终端、进程、工作区、浏览器和成果八个能力入口。
...
- 三种窗口宽度下布局可用。
- 八个标准能力在主要视图中固定可达，应用不按能力自动隐藏。
- 跨主视图切换保持栏目、折叠、跟随和固定目标状态。
- 任务中心、监督、Runtime、终端、进程、工作区、浏览器和成果更新不要求离开当前主任务。
- 键盘可操作，并具备正确 ARIA 标签。
...
- 分区记忆协同和唤起条件完成设计前，不新增相关数据、页面或任务转换。
