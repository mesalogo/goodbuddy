...
9. 当前来源失败时，不自动重试另一个来源。错误可以提供“前往通用设置更换下载源”，由
   用户明确选择。
10. 魔法笔记设置由应用中心与笔记页共享，不在全局设置中重复提供入口。
...
<PageTabs
  ...
  tabs={[
    ...
    { id: 'remote-projects', label: '远程项目（技术预览）' }
  ]}
  value={activeSection}
  variant="segmented"
/>
...
全局设置不再提供魔法笔记页签或跳转入口。应用中心与笔记页继续共享同一表单和配置，
保留已有启用、待办数量显示、评论方式、评论形式及业务数据。共同设置的交互与同步规则见
[应用导航 UI 设计](../features/application-tool-navigation/ui-design.md)。
...
- “平台功能”显示“通用设置”和“远程项目（技术预览）”两个可访问页签。
- 默认打开“通用设置”。
...
- 下载失败不会自动请求另一来源。
- 魔法笔记设置由应用中心与笔记页共享，全局设置不再提供笔记入口；已有配置值和业务数据保持不变。
