...
- 当前源码候选为 Desktop `0.13.12`、Agent `0.13.3`，OpenCode 固定为 `1.18.29`、Continue 为 `1.5.47`；正式发布状态以独立发布渠道为准。
...
- 发布准备执行定向回归、发布说明校验、类型检查及 lint。按用户要求不重跑本地全量测试；完整主分支 CI、生产构建、原生打包及公开发布核验待完成，本轮不做本地生产构建或打包。既有 Linux x64 生命周期验证使用 8 次真实请求，增量回顾验证使用 3 次，均不代表完整原生平台矩阵已验收。
