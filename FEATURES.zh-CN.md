...
- [x] **自动监督设置与范围**：“监督者 > 设置”是完整配置的唯一权威入口，支持创建和编辑 Global 或指定一个、多个 Project 的每日、每周计划；旧单项目配置无损迁移，项目级记忆与行动输出必须显式指定范围内的 Project。Task Center 和设置中心不再复制心跳表单；按分区回顾、候选生成和唤起条件仍待设计。详见 [智能心跳 PRD](./docs/features/smart-heartbeat/prd.md)。
...
- [ ] **监督后续能力**（规划中）：逐事件完整回放、Experiment 目标、用户取消、人工修改时间序列审计、事件触发及更完整的执行观察仍未完成。当前自动回顾受输入预算和滚动窗口限制，删除报告不会重置处理进度或重建图谱。详见 [会话监督 PRD](./docs/features/conversation-supervision/prd.md)。
...
- 当前 Desktop 候选为 `0.13.13`，已发布的 Agent `0.13.3` 保持不变，OpenCode 固定为 `1.18.29`、Continue 为 `1.5.47`；正式发布状态以独立发布渠道为准。
- Desktop `0.13.13` 减少流式活动历史保存时的重复读取，并修复监督反馈侧栏的紧凑固定/刷新按钮、目标名称和窄栏长标题显示。相较 `0.13.12` 不新增数据库格式迁移，无需更新 Host 环境。
- Desktop `0.13.12` 增加监督者回顾、图谱、活动和自动增量处理，减少流式活动写入，并改善 Mermaid、用量汇总、会话时长、Git、笔记和通知。数据库升级至 schema 46，回退旧客户端需使用升级前完整备份。
- Agent `0.13.3` 回收空闲 Runtime，并使被替换的 Agent 等已有工作结束后退出；复合包要求 Desktop `0.13.12`，请先升级 Desktop，再更新 Host 环境。Node 保持 `24.19.0`。
- 发布准备执行定向回归、发布说明校验、类型检查及 lint。按用户要求不重跑本地全量测试；本轮候选的完整主分支 CI、生产构建、原生打包及公开发布核验待完成，不做本地生产构建或打包。Desktop `0.13.12` 和 Agent `0.13.3` 已完成原生发布流程，但不替代开发期真实 Host 场景覆盖。
