...
- 手动回顾分页处理所选历史，并按批保存事实与来源；支持暂停和从保存进度继续。设置提供并发、超时、批次大小、执行预算和响应容量，活动记录显示实际阶段与已保存批次详情；工作回顾和活动记录使用面板可用全宽。
...
- 当前 Desktop 候选为 `0.13.15`，Agent `0.13.4` 已发布，OpenCode 固定为 `1.18.29`、Continue 为 `1.5.47`；正式发布状态以独立发布渠道为准。
...
- 发布准备要求全量测试、发布说明校验、类型检查及 lint；候选主分支 CI、生产构建、原生打包及公开发布核验为独立验收步骤。本轮不做本地生产构建、打包或龙芯预览，当前验证状态见[恢复版准备记录](./docs/development/release-preparation-0.13.15.md)。
