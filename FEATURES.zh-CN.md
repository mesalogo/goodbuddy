...
- 当前源码候选为 Desktop `0.13.14`、Agent `0.13.4`，OpenCode 固定为 `1.18.29`、Continue 为 `1.5.47`；正式发布状态以独立发布渠道为准。
- Desktop `0.13.14` 改善监督者分页回顾与续跑、ripgrep 原生搜索、待办布局和远程结果未知后的显式续聊。数据库升级至 schema 47，回退旧客户端需使用升级前完整备份。
- Agent `0.13.3` 回收空闲 Runtime，并使被替换的 Agent 等已有工作结束后退出；复合包要求 Desktop `0.13.12`，请先升级 Desktop，再更新 Host 环境。Node 保持 `24.19.0`。
- Agent `0.13.4` 在完成前等待模型交付确认，空闲 Runtime 回收失败不改变已完成结果，并在环境更新后清理已确认停用的旧程序及所属孤儿进程；保留历史、活动及状态不明的安装。独立包要求 Desktop `0.13.14`，请先升级 Desktop，再更新 Host。
- 发布准备要求本地全量测试、发布说明校验、类型检查及 lint；候选主分支 CI、生产构建、原生打包及公开发布核验为独立验收步骤。本轮不做本地生产构建、打包或龙芯预览，当前验证状态见[发布准备记录](./docs/development/release-preparation-0.13.14.md)。
