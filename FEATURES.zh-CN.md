...
- [x] **信创软硬件覆盖**：正式 Linux 安装包覆盖国产 `x64` 与 `arm64` 架构，可用于统信 UOS、银河麒麟及海光、兆芯、鲲鹏、飞腾等兼容环境。龙芯 LoongArch 另行提供 `loong64` 实验预览版，不属于正式发布矩阵，也未纳入自动更新；仅在明确要求时构建。这里的覆盖范围不等同于厂商或整机兼容认证，具体状态与限制见[龙芯预览版构建说明](./docs/development/loongarch-preview-build.md)。
...
- 待办宽屏列表与详情之间可拖动分隔条并记住宽度；来源笔记分组可独立折叠，不清除当前选中任务的详情。
...
- [x] **自动监督设置与范围**：“监督者 > 自动监督”统一管理计划、自动监督报告和建议，支持创建和编辑 Global 或指定一个、多个 Project 的每日、每周计划；工作回顾阅读带日期的统一监督结果及历史。旧单项目配置无损迁移，项目级记忆与行动输出必须显式指定范围内的 Project。Task Center 和设置中心不再复制心跳表单；按分区回顾、候选生成和唤起条件仍待设计。详见 [智能心跳 PRD](./docs/features/smart-heartbeat/prd.md)。
- [x] **监督者回顾、故事线图谱与活动**：按范围和时间回顾工作，查看保存结果的图谱及来源，确认或修订实体，预览写入本地知识库实体。侧栏可跟随或固定 Conversation、Task，并打开匹配结果。活动合并心跳报告与下游监督的阶段状态，保留失败和旧结果入口。自动阶段只处理新增、修改及未处理的来源片段，无变化时跳过模型；手动回顾重新整理有界历史。回顾只读且禁止工具，模型调用可能产生费用，详见[实施与验证记录](./docs/features/conversation-supervision/progress.md)。
- 手动回顾分页处理所选历史，并按批保存事实与来源；不再按固定时长自动暂停，仍支持主动暂停和从保存进度继续。设置提供并发、单次请求超时、批次大小和响应容量；活动记录突出相连阶段、已保存批次与剩余来源，次要统计可展开查看。工作回顾采用全宽纵向布局，运行状态单独显示，空态不再挤在控件旁。
...
- 当前 Desktop 候选为 `0.13.16`，Agent `0.13.4` 已发布，OpenCode 固定为 `1.18.29`、Continue 为 `1.5.47`；正式发布状态以独立发布渠道为准。
- Desktop `0.13.15` 承接未发布的 `0.13.14`，改善监督者分页回顾与续跑、ripgrep 原生搜索、待办布局和远程结果未知后的显式续聊。数据库升级至 schema 47，回退旧客户端需使用升级前完整备份。
- Desktop `0.13.16` 分离结构升级与旧历史回收，移除回顾固定时长自动暂停，并改善监督者布局。相较 `0.13.15` 不新增 schema 变更，也不要求更新 Agent。
- Agent `0.13.3` 回收空闲 Runtime，并使被替换的 Agent 等已有工作结束后退出；复合包要求 Desktop `0.13.12`，请先升级 Desktop，再更新 Host 环境。Node 保持 `24.19.0`。
- Agent `0.13.4` 在完成前等待模型交付确认，空闲 Runtime 回收失败不改变已完成结果，并在环境更新后清理已确认停用的旧程序及所属孤儿进程；保留历史、活动及状态不明的安装。独立包要求 Desktop `0.13.14` 或更新版本；因 `0.13.14` 未发布，请先升级至 Desktop `0.13.15`，再更新 Host。
- 发布准备要求全量测试、发布说明校验、类型检查及 lint；候选主分支 CI、生产构建、原生打包及公开发布核验为独立验收步骤。本轮不做本地生产构建、打包或龙芯预览，当前验证状态见[发布准备记录](./docs/development/release-preparation-0.13.16.md)。
- Desktop `0.13.12` 和 Agent `0.13.3` 已完成原生发布流程，但不替代开发期真实 Host 场景覆盖。
