...
- 品牌设置必须提供同布局预览、显式保存、默认 Logo 和整组恢复默认操作。自定义 Logo 仅
  ...
  单行截断，不能撑宽或挤压侧栏。
- 侧栏底部提供应用中心入口，设置按钮独立靠右。中心与共同应用设置使用和全局设置相同的整窗 Modal；魔法笔记、智能心跳、本机推理可启用、常驻与排序，
  取消常驻不关闭功能。知识库固定显示在首个可选应用槽位之后，再过滤隐藏的可选项；不参与启用、常驻或排序，中心只提供“打开”，原工作区实际配置保留，没有实际设置时不添加通用设置按钮。
  设置持久化后的事件同步各入口及工具配置写入；重开中心刷新期间锁定修改，新快照不被旧读取覆盖。文件、终端、浏览器和资源监控只通过右侧工作栏访问，不在左侧复制入口。
  具体规则见[应用中心 UI 设计](./docs/features/application-tool-navigation/ui-design.md)。已有稳定设置入口时，不在顶栏重复提供同一入口。
...
- 应用中心交付时，上述“平台功能 → 魔法笔记”的设置迁入应用共同设置界面；原页签如保留则只提供定位入口，不维护第二份表单。笔记页与应用中心共用配置；旧入口开关绑定业务启用值，迁为“启用应用”并保留原值，常驻使用独立配置。默认开启仅适用于未配置布尔值，只有独立显隐偏好才映射到常驻。此迁移规则见[应用导航 PRD](./docs/features/application-tool-navigation/prd.md#fr-11-应用导航设置)。
