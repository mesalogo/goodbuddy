...
- 左侧记录索引为 `168px`，开关独立位于详情标题左侧，提供 `aria-controls`、展开状态和动作提示；收起后完全移除索引列、间隙、边框及轨道，不占宽度，隐藏内容不可聚焦。右侧 AI 评论独立显示、滚动和收起，不与索引共享页签。每条记录一个导航项：画布显示真实首页缩略图及总页数，文字显示摘要，两者均显示类型和时间。两侧分别记住桌面展开状态，AI 另记宽度；宽布局分隔条保留 `separator` 语义及方向键、Home、End 操作。内容容器不超过 `800px` 时索引默认隐藏，由同一标题栏按钮开闭标题栏下方的 `168px` 抽屉，标题栏自身保留控件高度，定位不依赖新增按钮；不保留轨道。选择记录或按 Escape 后关闭，Escape 将焦点返回开关。AI 移至底部，高度不超过可用布局的 40% 且最多 `280px`，不显示分隔条。
...
- 解析结果使用统一文档结构，至少保留文档标题、来源格式、页码或工作表定位、正文块、置信度、处理方式和警告。聊天附件保留完整解析文本，超过暂存容量时报错；知识库使用完整结果分块和索引，具体要求见[文档处理 PRD](./docs/features/document-processing/prd.md#32-场景工作流)。
