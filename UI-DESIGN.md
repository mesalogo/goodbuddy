...
- 共享锚定菜单 `AnchoredMenu` 和应用通知共用 `FloatingPortal` 的非模态 `popover="manual"`，由浏览器 top layer
  绘制，避免被 Modal 或滚动容器遮挡。菜单挂在触发按钮所属 Modal 内，通知跟随当前活动
...
- 文字记录编辑器保留 Quill 原生列表快捷输入：输入 `-`、`*`、`1.`、`[ ]` 或 `[x]` 后按空格，分别转换为项目符号、编号或待办清单。快捷输入只生成现有 Delta 富文本，不引入 Markdown 源码模式、第二套文字记录持久化格式或第三方 Markdown 插件；编辑区附近持续提供简短提示。
...
- 魔法笔记设置提供“发送画布页数”单选框，复用设置表单 `.field` 控件样式，范围 1～8，默认 1。标题旁的问号图标通过现有 `title` 提示展示说明，不常驻整段文字；下拉框与帮助图标均通过 `aria-describedby` 关联屏幕阅读器说明。说明包含当前页序前 N 页、不足时发送全部页、数量指单个画布页数而非记录数，以及完整保存与导出不受影响。修改后沿用设置保存反馈，下次分析生效。
