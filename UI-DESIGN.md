...
- 工作栏开关位于窗口关闭按钮下方的当前任务或页面上下文栏最右侧，同一个按钮负责展开与
  收起并持续暴露状态；图标中的箭头方向必须随当前动作切换，收起状态提示展开，展开状态
  提示收起。按钮保持普通图标按钮视觉，不通过持续高亮表达展开状态。工作栏内部不再重复
  关闭按钮。上下文栏应为后续任务状态、范围和高频操作预留空间，不能只作为孤立的按钮容器。
...
- 无边框窗口使用两层顶部结构：`30px` 窗口标题栏只承载拖动区域和窗口控制；其下
  `46px` 当前任务或页面上下文栏承载侧栏开关、标题、范围、状态和高频操作。普通图标按钮
  使用 `34px × 34px` 点击区域。
