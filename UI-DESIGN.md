...
- 笔记详情的标题、正文和记录流不再包裹额外的大外框，记录流容器不加内边距；保留编辑器自身边框、页面安全边距及 AI 区域分隔。此规则仅作用于笔记详情，不改变卡片总览与待办独立布局。
- 画布新建、编辑和只读态的内框统一保留 `12px` 间距，避免内外边框及圆角重叠。记录头部操作按内容宽度排列，空间不足时自然换行。文字新建的保存区与编辑器属于同一卡片，空白编辑区使用最小高度，不随容器百分比高度拉长。文字工具栏图标、菜单和原生滚动条须跟随主题。
