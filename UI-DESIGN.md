...
- “笔记 / 待办”属于同一工作台内的同级内容面板，使用 `PageTabs` 的 `segmented` 视觉变体，放在页头“新建笔记”按钮左边，不另占一行导航。按内容宽度显示，不拉伸；窄窗口操作组可换行到标题下方，保留控件高度。
...
- 笔记详情保留可隐藏的 AI 点评栏，宽度和显示状态记住用户选择。宽布局分隔条保留 `separator` 语义及方向键、Home、End 操作；内容容器不超过 `800px` 时 AI 移至底部，高度不超过可用布局的 40% 且最多 `280px`，不显示分隔条。
