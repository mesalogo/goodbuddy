...
describe('desktop packages workflow', () => {
  it('prepares a sandbox and virtual display before running Electron tests', () => {
    ...
    expect(steps[prepareIndex]?.run).toContain('if [[ ! -f "$sandbox" ]]')
    expect(steps[prepareIndex]?.run).toContain('force_no_cache=true node node_modules/electron/install.js')
    expect(steps[prepareIndex]?.run).toContain('test -f "$sandbox"')
    expect(steps[prepareIndex]?.run).toContain('sudo chown root:root "$sandbox"')
    expect(steps[prepareIndex]?.run).toContain('sudo chmod 4755 "$sandbox"')
